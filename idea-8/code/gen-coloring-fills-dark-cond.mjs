// EXPERIMENT (idea #8) — temporary variant of gen-coloring-fills-dark.mjs that
// ALSO conditions the night generation on the page's committed LIGHT raw as a
// second input image, asking the model to keep each object's hue family
// (dimmed and cooled) so light and night palettes cohere. Writes to
// .coloring-samples-dark/cond/. DELETE AFTER THE EXPERIMENT.
//
...
const OUT_DIR = join(SAMPLES_DARK_DIR, 'cond');
let apiCalls = 0;

// The palette-coherence instruction — only used when the page has a committed
// light raw to hand the model as a second image.
const PALETTE_REF = `\n\nPALETTE REFERENCE — THE SECOND IMAGE:
You are also given a SECOND image: the SAME drawing colored as a bright DAYTIME scene. Use it ONLY as a color reference. For every object and region, keep the SAME hue family the daytime image uses — a red-and-white blanket stays red-and-white, a yellow flower stays yellow, a green leaf stays green — just DIMMED and COOLED for night. Never swap an object's color family away from what the daytime image shows. Do NOT copy the daytime image's brightness or its sky: the background still becomes a deep evening-sky tone as described above. Your output's line work must match the FIRST image (the white-line drawing) exactly — the second image is for colors only. CRITICAL: the daytime reference is shown WITHOUT line art — do not take any line-work cues from it. Every outline in YOUR image must be a BRIGHT WHITE line exactly as in the first image; a result with black or dark outlines is WRONG and unusable.`;
...
async function generateDarkPage(ai, { imageBytes, mimeType, temperature, chalked, lightRef }) {
  let base = darkFillPrompt(chalked);
  if (lightRef) base += PALETTE_REF;
            ...
            ? [
                {
                  inlineData: {
                    mimeType: 'image/webp',
                    data: Buffer.from(lightRef).toString('base64'),
                  },
                },
              ]
            : []),
...
for (const page of pages) {
  ...
  // Palette reference = the SHIPPED punched light fill (fills-only, outlines
  // inpainted away) — the raw's black outlines tempt the model into re-inking
  // the night take with dark lines.
  const lightPunchedPath = join(COLORING_DIR, `${rel}.light.webp`);
  const lightPunched = existsSync(lightPunchedPath) ? await readFile(lightPunchedPath) : null;
