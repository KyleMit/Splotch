...
Output:
    benchmark/corpus/<nn>-<name>.svg      filled shape, one <path> per element
    benchmark/corpus/corpus.json          ground truth
"""
...
OUT = CAPABILITY_ROOT / "benchmark" / "corpus"
...
def variable_stroke(points, r0, r1) -> Polygon:
    ...
    discs = [
        ...
        for p, ti in zip(pts, t, strict=True)
    ]
...
def build() -> list[dict]:
    ...
    add(16, "y-junction", [unary_union([stroke(leg) for leg in y_legs])], y_legs)
...
def write(cases: list[dict]) -> dict:
    ...
    for case in cases:
        ...
        manifest["cases"].append({
            ...
            "num": case["num"],
            "name": case["name"],
            "svg": f"benchmark/corpus/{case['id']}.svg",
            "notes": case["notes"],
            "radius": case["radius"],
