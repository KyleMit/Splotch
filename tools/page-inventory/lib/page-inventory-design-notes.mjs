...
export const GENERAL_DESIGN_NOTES = [
  ...
  'Several overlays here are dialogs that lay one scrim over the whole page behind them — the settings card, the color picker, the coloring-book and AI cards, the parent gate, the admin action sheet. That scrim is a single sheet with nothing above it: it darkens and blurs the canvas, the strip of color swatches along the top edge (the left edge in landscape), the docked trash button, the bottom-corner button panel, and the corner glyphs all at once, in both themes, so the open card holds attention. The swatches and the trash button are saturated colors, so under the scrim they still read as colored while the paper reads gray, and the swatch strip keeps its own crisp edge against the paper; a band of color or a hard seam there is those elements showing through the same scrim as everything else, not a gap in it or a layer it missed. The floor a scrim owes is that it darkens everything outside the card: report its coverage only when a region outside the open card shows no darkening at all — a white strip still white, paper still bright — rather than when it looks brighter than the paper beside it. Where you can see that the page behind an overlay has been darkened that way, background content reading dim, gray, soft, or blurred is that scrim rather than the styling of the elements beneath it, and the contrast and legibility to judge are those of the open card — its text, its controls, its close button — together with anything drawn on top of the scrim. The brush and stroke-width flyouts cast no scrim: where the page behind an overlay is still at full brightness, every element in the frame keeps its usual scope.',
...
const EMPTY_FEEDBACK_FORM_NOTE =
  'The "Send report" button stays disabled until the message box has text, and this capture shows the form before anything has been typed, so that button is in its disabled state here. A disabled button is drawn at half opacity, fill and label together, which is how this app marks the action unavailable; that half-strength appearance is the disabled styling arriving and is not a claim about the button’s contrast once a message is typed and it is enabled. The floor it still owes is that its label remains readable: report it only when you cannot read the words "Send report" on it.';

export const SURFACE_DESIGN_NOTES = {
  'routes/feedback': EMPTY_FEEDBACK_FORM_NOTE,
  'settings/settings-feedback': EMPTY_FEEDBACK_FORM_NOTE,
