...
export function auditImageReports({
  root = ROOT,
  snapshotId = runId(),
  command = runNetlify,
} = {}) {
...
async function main() {
  ...
  const result = auditImageReports();
  ...
  console.log(`[audit-reports] site: ${result.site.name} (${PRODUCTION_DOMAIN})`);
  console.log(
    `[audit-reports] fetched ${result.reports.length} report(s) to ${relative(ROOT, result.snapshotDir)}`
  );
  console.log(
    `[audit-reports] model-eval inputs: ${copied} copied, ${unchanged} unchanged, ${skipped} skipped`
  );
  if (result.reports.length) console.log('[audit-reports] run: FILTER=report__ npm run model-eval');
}
