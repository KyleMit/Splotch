...
const OUT = '/tmp/claude-0/-home-user-Splotch/68ded56b-e7dd-5cff-b995-afd9f1565152/scratchpad/ideas/idea-12/instr';
...
const region = { left: Math.max(0, x - r), top: Math.max(0, y - r), width: Math.min(2*r, meta.width - (x-r)), height: Math.min(2*r, meta.height - (y-r)) };
const slug = rel.replace('/', '_');
const tiles = [];
for (const [name, buf] of [['pen', pen], ['chalk', chalk], ['light', light], ['night', night]]) {
  if (!buf) continue;
  const full = await sharp(buf).removeAlpha().resize(meta.width, meta.height, { fit: 'fill' }).png().toBuffer();
  const t = await sharp(full).extract(region).resize(260, 260, { fit: 'inside' }).png().toBuffer();
  tiles.push({ name, t });
}
const canvas = sharp({ create: { width: 270 * tiles.length, height: 280, channels: 3, background: '#888' } });
await canvas.composite(tiles.map((t, i) => ({ input: t.t, left: 5 + i * 270, top: 10 }))).webp().toFile(join(OUT, `${slug}_${x}x${y}.crops.webp`));
