...
for (const r of results) {
  ...
  r.rawMax = r.localMax;
  const worstConf = conf.length
    ? conf.reduce((a, b) => (b.local > a.local ? b : a))
    : null;
...
for (const theme of ["night", "light"]) {
  ...
  const beats = big.map(
    (r) => rest.filter((o) => r.confMax > o.confMax).length / rest.length,
  );
  console.log(
...
const shifted = results.filter(
  (r) => Math.abs(r.globalDx) + Math.abs(r.globalDy) >= 2,
);
for (const r of shifted.sort(
  (a, b) =>
    Math.hypot(b.globalDx, b.globalDy) - Math.hypot(a.globalDx, a.globalDy),
))
  console.log(`  ${r.page} ${r.theme}: (${r.globalDx},${r.globalDy})`);
console.log(
  `  ${shifted.length}/${results.length} raws with |residual| >= 2px`,
);
