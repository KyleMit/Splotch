...
// (audit-golden.mjs, golden/golden-scores.json) deliberately leave out.
//
...
//
//   npm run gen:assets:manifest       rewrite tools/asset-gen/golden/asset-manifest.sha256
...
if (!checkMode) {
  ...
  console.log(
    `[asset-manifest] wrote ${entries.length} asset hash(es) to tools/asset-gen/golden/asset-manifest.sha256`
  );
  process.exit(0);
}
...
if (problems.length) {
  ...
  fail(
    `[asset-manifest] ${problems.length} asset(s) drifted from the manifest. If the change is ` +
      ...
      'assets changed (a night-only pass may only touch *.night/*.chalk/*.chalk.thumb lines, ' +
      'never a *.light/*.outline line or a pen *.thumb line).'
  );
}
