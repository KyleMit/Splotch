...
3.1's faithfulness resists erase-style edits on solid ink (ISSUES #9 caveat), while a fresh
...
* The ~30 remaining flat-eye flags — a mix of detector noise on non-face cores (ISSUES #4) and
...
* A fresh drawing re-rolls **everything** — composition, motifs, palette anchors.
  Sibling-orientation consistency (ISSUES #12) gets worse, not better, unless the `--scene` names
