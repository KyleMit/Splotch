...
* **npm workspace.** The obvious "monorepo module" mechanism, but it **fights ADR-0029's
  load-bearing invariant**: `cap sync` read plugin code out of a **flat** `node_modules`. npm
...
* **Plain in-repo folder, dependencies staying in the root `package.json`.** Leaves `node_modules`
  byte-for-byte unchanged — so ADR-0029, ADR-0011's `postinstall`, `cap sync`, are entirely
...
* \+ **`node_modules` is unchanged**, so ADR-0029's flat-tree invariant, ADR-0011's `postinstall`,
  `cap sync`, keep working with zero new configuration. The app build path never touches the folder.
