...
`gen-coloring-thumbs`, `gen-coloring-sheet` (since retired — its review role folded into the contact
sheet), `gen-contact-sheet`, `png-to-webp`, `lib/pixelate.mjs`, and the `night-fills.md` runbook.
...
* Entry-point scripts in `scripts/`, docs (the `README.md` runbook, `pipeline.md`, the decision
  records) in `docs/`, plus `CLAUDE.md` (scoped rules) at the folder root.
