...
5. `npm run check:coloring-assets` + `npm run check` + `npm run test:unit` + `npm run build`,
   ...
   stay out in favor of their picker thumbnails. Web compact packs map the raster fills to their
   1,152 px derivatives; the canvas and export path use the canonical SVG on every platform, so no
   pack carries a presentation WebP (ADR-0152 retired them, and a 2026-09-02 physical-iPad A/B of
   lossless presentation tiers changed no scored frame — PR 1553).
