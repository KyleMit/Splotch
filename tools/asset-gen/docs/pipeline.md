...
| `{page}.{dark.}presentation.webp` | `web/static/coloring/max-{edge}px/{book}/` | web only — deterministic lossless paper presentation tiers at 1152/1536/2304/3072 px max edge (whole-number scales of the SVG viewBox), the web paper's `srcset` candidates; the SVG stays the export, Magic, and native presentation authority | `gen-responsive-assets.mjs` from canonical SVG, bound to the SVG digest in `golden/presentation-sources.json` |
...
5. `npm run check:coloring-assets` + `npm run check` + `npm run test:unit` + `npm run build`,
   ...
   stay out in favor of their picker thumbnails. Web packs also carry the paper presentation tiers
   under their hosted tier URLs — compact the 1152 px tier, full the whole ladder — so the paper's
   `srcset` pick is served offline; native packs keep selector rasters and omit presentation WebPs
   because the native canvas and every export path use the canonical SVG.
