...
cross-checked against the pen. Burn these down before any other tier: #1–#2 are night-fill-only
regens (the cheapest fix in the pipeline), #3–#5 need a chalk regen first, and #6 is the one page
needing pen work. With one exception (#2, which the halo auditor ranks but nothing reviews), every
item passes all its gates (`gen:coloring-fills:audit:eyes` → ok on every listed page), so nothing
...
1. **`vehicles/garbage-tall`'s night fill drew a leaf on the trash can** *(shipped asset)*: the pen,
   ...
   The invented-shapes audit (#10) only scans the open background, so subject-interior invention is
   invisible from either layer. Night-fill regen with the seeded no-emblem notes + re-punch.
   (Unrelated but noted: this page is on #12's light-side flat-eye list — its cab face fails the
   light audit — so a regen wave touching it should consult both entries.)
2. **`vehicles/excavator-tall` ships noisy static around its night lines — and the halo auditor
   ...
   `objects/house-tall` 2.5, `vehicles/fire-tall` 2.4, `objects/house-wide` 2.2 — then a clean
   break to ≤1.3. (`objects/flower-wide`, formerly 4.0 in this cluster, dropped to 0.2 with its
   2026-07-13 chalk+night regen.) The auditor is deterministic and offline but "a ranking for crop
   review, not a verdict": nothing reviews its top before shipping, which is how the #1 and #2
   pages shipped. Fix: night regen for excavator-tall (seeded no-re-inked-lines notes), crop-review
   the rest of the ≥2.2 cluster, and decide a haloScore threshold so the next wave gates on it.
3. **The chalk invents facial features — teeth on `farm/duck-wide`, whole faces on
   ...
   Triple gate-blind: the invented-shapes audit (#10) only scans the open background, the eye audit
   ...
   guard), then night regen + re-punch — mind #12's caveat (3.1 resists erase edits on solid ink;
...
4. **`farm/dog-wide` glows white at night — nose, eye whites, rump patch** *(shipped asset)*: the
   ...
   look. This is the #11 class escaping the eye: the chalk whitens *any* solid-pen region, and here
...
5. **Chalk-whitened night eyes on `farm/horse-wide`, `farm/horse-tall`, `nature/caterpillar-wide`,
   ...
   five more #11 composites in the wild. The horses are textbook — their pens have solid-ink pupils,
...
6. **`creatures/mermaid-tall`'s light eyes are giant solid-black orbs** *(shipped asset)*: the pen
   draws each pupil as solid ink filling nearly the whole eye (only two catchlight holes), so the
   light fill has nothing to paint and the face reads dead-eyed in light mode. This is #12's
   ...
   for #12's audit-driven wave. The one Tier 1 item needing pen work: normalize the pen
   (`gen:coloring-outlines:normalize`, seeded notes) + light-fill regen — mind #12's caveat (the
...
7. **`judgeLightEyes` has no false-positive suppressions** *(gate blind spot)*. The IDEAS #12 fixes
   ...
   this before #12 — it de-noises the flat-eye list so the burn-down only spends API budget on real
   offenders.
8. **The orphan pages are still uncataloged (IDEAS #24)** *(shipped asset)*: `shapes/heart-wide`
...
9. **Night subject/background contrast is unmeasured** *(gate blind spot)* (`shapes/circle-wide`
   ...
   *measures* the result — the gate gap stands. The 2026-07-13 night-eye fix on
   ...
   pages are fixed, the gap is not. The 2026-07-13 `objects/flower-wide` regen hit it again at
   motif scale — an unpinned take painted its two small background flowers sky-navy and passed
   every gate; caught by eye and fixed with a palette pin, the gap stays.)
10. **Colored-shape invention is only audited, not gated (IDEAS #13)** *(gate blind spot)*: the
    ...
    eyeballs, and Tier 1 #1's fill-side leaf, all sit *inside* the subject, where neither this audit
    nor any gate looks.

## Tier 3 — high impact but expensive

11. **Chalk whitening on solid-pen-eye pages is gate-blind** *(gate blind spot)* (proved by
    ...
    face positions. No ready patch — worth building before #12's burn-down, since that wave
    ...
    the chalk whitened a solid nose and even solidified an *outlined* rump patch; Tier 1 #5's
    ...
    opening, not the fill, decides where a pupil can appear). So a scorer keyed to
    "pen solid regions" misses all three extensions — compare chalk ink against PEN ink per region,
    and flag chalk ink the pen doesn't have (which would also catch Tier 1 #3's invented features).
12. **Light-mode eyes on accident-era pens are dead/solid** *(shipped asset; IDEAS #6 — the biggest
    ...
    2026-07-13 fresh-outline pass) — though some are detector noise, not defects, which is why #7
    ...
    2026-07-13 (its page fix is prioritized as Tier 1 #6). The root cause is the pen: a solid-ink
    ...
    should get a light-only treatment, not a fresh drawing. Night mode is unaffected (the chalk
    owns those whites). **Caveat:** 3.1 resists erase-style edits on solid pen ink — its
    faithfulness works against radical whitening/erase edits (police-tall took an explicit erase
    `--notes`; a 2.5-era chalk did the same edit unprompted). The pen normalizer is exactly this
    kind of edit and has NOT been exercised on 3.1 yet — budget extra attempts/notes the first
    time.
13. **Style covers are still 2.5-era outputs** *(shipped asset)*. The 3.1 migration swapped the
...
next mass regen happens. (The "nothing shipped looks wrong" framing died 2026-07-13: #14's gap
...
14. **Light↔night and tall↔wide palette coherence are unenforced (IDEAS #8/#9)** *(gate blind
    ...
    *identity* color (`nature/ant-tall`'s night ant turned green; the page was fixed 2026-07-13 by
    a seeded-notes night regen, but only human review caught it — the gap stays open). The hue-flip
...
15. **Motif consistency across sibling pages is unenforced (IDEAS #2)** *(gate blind spot)*: the
