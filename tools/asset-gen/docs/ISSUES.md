...
cross-checked against the pen. Burn these down before any other tier: #1–#5 are night-fill-only
regens (the cheapest fix in the pipeline), #6–#8 need a chalk regen first, and #9 is the one page
needing pen work. With one exception (#5, which the halo auditor ranks but nothing reviews), every
...
2. **`nature/ant-tall`'s night ant is green** *(shipped asset)*: the light fill is a proper
   ...
   subject-color plausibility. Night-fill regen with the seeded "stay reddish-brown like the light
   fill" notes + re-punch.
3. **`objects/flower-wide`'s night face is cross-eyed** *(shipped asset)*: the night fill painted
   ...
   notes + re-punch.
4. **`vehicles/garbage-tall`'s night fill drew a leaf on the trash can** *(shipped asset)*: the pen,
   ...
   first confirmed FILL-side invention inside the subject (every #6 invention came from the chalk).
   The invented-shapes audit (#13) only scans the open background, so subject-interior invention is
   invisible from either layer. Night-fill regen with the seeded no-emblem notes + re-punch.
   (Unrelated but noted: this page is on #15's light-side flat-eye list — its cab face fails the
   light audit — so a regen wave touching it should consult both entries.)
5. **`vehicles/excavator-tall` ships noisy static around its night lines — and the halo auditor
   ...
   `space/ship-tall` 8.5 (#6's porthole page — its regen covers this too), excavator-tall 6.5,
   `shapes/rectangle-tall` 4.3, `objects/flower-wide` 4.0 (#3's page), `shapes/heart-tall` 3.2,
   ...
   wave gates on it.
6. **The chalk invents facial features — teeth on `farm/duck-wide`, whole faces on
   ...
   Triple gate-blind: the invented-shapes audit (#13) only scans the open background, the eye audit
   ...
   guard), then night regen + re-punch — mind #15's caveat (3.1 resists erase edits on solid ink;
   budget extra attempts/notes).
7. **`farm/dog-wide` glows white at night — nose, eye whites, rump patch** *(shipped asset)*: the
   ...
   look. This is the #14 class escaping the eye: the chalk whitens *any* solid-pen region, and here
...
8. **Chalk-whitened night eyes on `farm/horse-wide`, `farm/horse-tall`, `nature/caterpillar-wide`,
   `objects/teddy-tall`, and hollow circle catchlights on `nature/bee-tall`** *(shipped asset)*:
   five more #14 composites in the wild. The horses are textbook — their pens have solid-ink pupils,
...
9. **`creatures/mermaid-tall`'s light eyes are giant solid-black orbs** *(shipped asset)*: the pen
   draws each pupil as solid ink filling nearly the whole eye (only two catchlight holes), so the
   light fill has nothing to paint and the face reads dead-eyed in light mode. This is #15's
   ...
   for #15's audit-driven wave. The one Tier 1 item needing pen work: normalize the pen
   (`gen:coloring-outlines:normalize`, seeded notes) + light-fill regen — mind #15's caveat (the
...
10. **`judgeLightEyes` has no false-positive suppressions** *(gate blind spot)*. The IDEAS #12 fixes
    (band-blind annulus, chalk-white-nearby) apply only to the night judge, so light-side flags
    still fire on side-profile eyes (`farm/duck-wide`, verified lively), band-blind solid-pupil
    pages, and non-face cores (windows, hubs — `objects/house-tall`). Options: port the band-blind
    rule, or bless per-page eye annotations
    (`ideas-exploration/idea-12/code/eye-annotations.draft.json`). Do this before #15 — it de-noises
    the flat-eye list so the burn-down only spends API budget on real offenders.
11. **The orphan pages are still uncataloged (IDEAS #24)** *(shipped asset)*: `shapes/heart-wide`
...
12. **Night subject/background contrast is unmeasured** *(gate blind spot)* (`shapes/circle-wide`
...
13. **Colored-shape invention is only audited, not gated (IDEAS #13)** *(gate blind spot)*: the
    ...
    its scope is the open background only — Tier 1 #6's invented teeth, apple faces, and porthole
    eyeballs, and Tier 1 #4's fill-side leaf, all sit *inside* the subject, where neither this audit
    nor any gate looks.

## Tier 3 — high impact but expensive

14. **Chalk whitening on solid-pen-eye pages is gate-blind** *(gate blind spot)* (proved by
    ...
    solid-pen-eye page: Tier 1 #8's horses are two of them). A solid pen pupil has no nested rings →
    ...
    face positions. No ready patch — worth building before #15's burn-down, since that wave
    ...
    prove the class is wider than its name: Tier 1 #7 (`farm/dog-wide`) shows it isn't eye-shaped —
    the chalk whitened a solid nose and even solidified an *outlined* rump patch — and Tier 1 #8's
    ...
    flag chalk ink the pen doesn't have (which would also catch Tier 1 #6's invented features).
15. **Light-mode eyes on accident-era pens are dead/solid** *(shipped asset; IDEAS #6 — the biggest
    ...
    2026-07-13 fresh-outline pass) — though some are detector noise, not defects, which is why #10
    ...
    2026-07-13 (its page fix is prioritized as Tier 1 #9). The root cause is the pen: a solid-ink
...
16. **Style covers are still 2.5-era outputs** *(shipped asset)*. The 3.1 migration swapped the
...
green ant is #17's gap shipping a real defect. The page fix is Tier 1; the gate stays here.)

17. **Light↔night and tall↔wide palette coherence are unenforced (IDEAS #8/#9)** *(gate blind
...
18. **Motif consistency across sibling pages is unenforced (IDEAS #2)** *(gate blind spot)*: the
