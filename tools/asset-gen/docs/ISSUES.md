...
cross-checked against the pen. Burn these down before any other tier: #1–#3 are night-fill-only
regens (the cheapest fix in the pipeline), #4–#6 need a chalk regen first, and #7 is the one page
needing pen work. With one exception (#3, which the halo auditor ranks but nothing reviews), every
...
1. **`objects/flower-wide`'s night face is cross-eyed** *(shipped asset)*: the night fill painted
   ...
   notes + re-punch.
2. **`vehicles/garbage-tall`'s night fill drew a leaf on the trash can** *(shipped asset)*: the pen,
   ...
   first confirmed FILL-side invention inside the subject (every #4 invention came from the chalk).
   The invented-shapes audit (#11) only scans the open background, so subject-interior invention is
   invisible from either layer. Night-fill regen with the seeded no-emblem notes + re-punch.
   (Unrelated but noted: this page is on #13's light-side flat-eye list — its cab face fails the
   light audit — so a regen wave touching it should consult both entries.)
3. **`vehicles/excavator-tall` ships noisy static around its night lines — and the halo auditor
   ...
   `space/ship-tall` 8.5 (#4's porthole page — its regen covers this too), excavator-tall 6.5,
   `shapes/rectangle-tall` 4.3, `objects/flower-wide` 4.0 (#1's page), `shapes/heart-tall` 3.2,
   ...
   wave gates on it.
4. **The chalk invents facial features — teeth on `farm/duck-wide`, whole faces on
   ...
   Triple gate-blind: the invented-shapes audit (#11) only scans the open background, the eye audit
   ...
   guard), then night regen + re-punch — mind #13's caveat (3.1 resists erase edits on solid ink;
   budget extra attempts/notes).
5. **`farm/dog-wide` glows white at night — nose, eye whites, rump patch** *(shipped asset)*: the
   ...
   look. This is the #12 class escaping the eye: the chalk whitens *any* solid-pen region, and here
...
6. **Chalk-whitened night eyes on `farm/horse-wide`, `farm/horse-tall`, `nature/caterpillar-wide`,
   `objects/teddy-tall`, and hollow circle catchlights on `nature/bee-tall`** *(shipped asset)*:
   five more #12 composites in the wild. The horses are textbook — their pens have solid-ink pupils,
...
7. **`creatures/mermaid-tall`'s light eyes are giant solid-black orbs** *(shipped asset)*: the pen
   draws each pupil as solid ink filling nearly the whole eye (only two catchlight holes), so the
   light fill has nothing to paint and the face reads dead-eyed in light mode. This is #13's
   ...
   for #13's audit-driven wave. The one Tier 1 item needing pen work: normalize the pen
   (`gen:coloring-outlines:normalize`, seeded notes) + light-fill regen — mind #13's caveat (the
...
8. **`judgeLightEyes` has no false-positive suppressions** *(gate blind spot)*. The IDEAS #12 fixes
   ...
   this before #13 — it de-noises the flat-eye list so the burn-down only spends API budget on real
   offenders.
9. **The orphan pages are still uncataloged (IDEAS #24)** *(shipped asset)*: `shapes/heart-wide`
...
10. **Night subject/background contrast is unmeasured** *(gate blind spot)* (`shapes/circle-wide`
    ...
    *measures* the result — the gate gap stands. The 2026-07-13 night-eye fix on
    ...
    pages are fixed, the gap is not.)
11. **Colored-shape invention is only audited, not gated (IDEAS #13)** *(gate blind spot)*: the
    ...
    its scope is the open background only — Tier 1 #4's invented teeth, apple faces, and porthole
    eyeballs, and Tier 1 #2's fill-side leaf, all sit *inside* the subject, where neither this audit
    nor any gate looks.

## Tier 3 — high impact but expensive

12. **Chalk whitening on solid-pen-eye pages is gate-blind** *(gate blind spot)* (proved by
    ...
    solid-pen-eye page: Tier 1 #6's horses are two of them). A solid pen pupil has no nested rings →
    ...
    face positions. No ready patch — worth building before #13's burn-down, since that wave
    ...
    prove the class is wider than its name: Tier 1 #5 (`farm/dog-wide`) shows it isn't eye-shaped —
    the chalk whitened a solid nose and even solidified an *outlined* rump patch — and Tier 1 #6's
    ...
    flag chalk ink the pen doesn't have (which would also catch Tier 1 #4's invented features).
13. **Light-mode eyes on accident-era pens are dead/solid** *(shipped asset; IDEAS #6 — the biggest
    ...
    2026-07-13 fresh-outline pass) — though some are detector noise, not defects, which is why #8
    ...
    2026-07-13 (its page fix is prioritized as Tier 1 #7). The root cause is the pen: a solid-ink
...
14. **Style covers are still 2.5-era outputs** *(shipped asset)*. The 3.1 migration swapped the
...
next mass regen happens. (The "nothing shipped looks wrong" framing died 2026-07-13: #15's gap
...
15. **Light↔night and tall↔wide palette coherence are unenforced (IDEAS #8/#9)** *(gate blind
...
16. **Motif consistency across sibling pages is unenforced (IDEAS #2)** *(gate blind spot)*: the
