...
cross-checked against the pen. Burn these down before any other tier: #1–#2 need a chalk regen
first, and #3 is the one page needing pen work. Every item passes all its gates
...
Tier 2 #4. The tier's chalk-invented-faces item — teeth on `farm/duck-wide`, whole faces on both
...
folded into Tier 3 #9 and the per-page recipes live in the farm/objects/space registries.)

1. **`farm/dog-wide` glows white at night — nose, eye whites, rump patch** *(shipped asset)*: the
   ...
   look. This is the #9 class escaping the eye: the chalk whitens *any* solid-pen region, and here
...
2. **Chalk-whitened night eyes on `farm/horse-wide`, `farm/horse-tall`, `nature/caterpillar-wide`,
   `objects/teddy-tall`, and hollow circle catchlights on `nature/bee-tall`** *(shipped asset)*:
   five more #9 composites in the wild. The horses are textbook — their pens have solid-ink pupils,
...
3. **`creatures/mermaid-tall`'s light eyes are giant solid-black orbs** *(shipped asset)*: the pen
   draws each pupil as solid ink filling nearly the whole eye (only two catchlight holes), so the
   light fill has nothing to paint and the face reads dead-eyed in light mode. This is #10's
   ...
   for #10's audit-driven wave. The one Tier 1 item needing pen work: normalize the pen
   (`gen:coloring-outlines:normalize`, seeded notes) + light-fill regen — mind #10's caveat (the
...
4. **The halo ranking is crop-reviewed but still ungated — wire haloScore into the night generator**
...
5. **`judgeLightEyes` has no false-positive suppressions** *(gate blind spot)*. The IDEAS #12 fixes
   ...
   this before #10 — it de-noises the flat-eye list so the burn-down only spends API budget on real
   offenders.
6. **The orphan pages are still uncataloged (IDEAS #24)** *(shipped asset)*: `shapes/heart-wide` and
...
7. **Night subject/background contrast is unmeasured** *(gate blind spot)* (`shapes/circle-wide`
...
8. **Colored-shape invention is only audited, not gated (IDEAS #13)** *(gate blind spot)*: the
   ...
   porthole eyeballs (Tier 3 #9's case history) all sat *inside* the subject, where neither this
...
9. **Chalk whitening on solid-pen-eye pages is gate-blind** *(gate blind spot)* (proved by
   ...
   solid-pen-eye page: Tier 1 #2's horses are two of them). A solid pen pupil has no nested rings →
   ...
   face positions. No ready patch — worth building before #10's burn-down, since that wave
   ...
   prove the class is wider than its name: Tier 1 #1 (`farm/dog-wide`) shows it isn't eye-shaped —
   the chalk whitened a solid nose and even solidified an *outlined* rump patch; Tier 1 #2's
   ...
   chalk ink the pen doesn't have.
   ...
   **chalk-invented-faces class** (teeth in `farm/duck-wide`'s bill; whole faces on
   ...
   is triple gate-blind: the invented-shapes audit (#8) scans only the open background; the eye
   ...
   feature.
10. **Light-mode eyes on accident-era pens are dead/solid** *(shipped asset; IDEAS #6 — the biggest
    ...
    2026-07-13 fresh-outline pass) — though some are detector noise, not defects, which is why #5
    ...
    2026-07-13 (its page fix is prioritized as Tier 1 #3). The root cause is the pen: a solid-ink
...
11. **Style covers are still 2.5-era outputs** *(shipped asset)*. The 3.1 migration swapped the
...
next mass regen happens. (The "nothing shipped looks wrong" framing died 2026-07-13: #12's gap
...
12. **Light↔night and tall↔wide palette coherence are unenforced (IDEAS #8/#9)** *(gate blind
...
13. **Motif consistency across sibling pages is unenforced (IDEAS #2)** *(gate blind spot)*: the
