...
cross-checked against the pen. Burn these down before any other tier: #1–#4 are night-fill-only
regens (the cheapest fix in the pipeline), #5–#7 need a chalk regen first, and #8 is the one page
needing pen work. With one exception (#4, which the halo auditor ranks but nothing reviews), every
...
1. **The night eye reads as a blank white orb on `dinosaur/velociraptor-wide`** *(shipped asset)*:
   ...
   eye is a white ring around a hollow — no legible dark pupil. This is the #11 contrast gap at eye
   ...
   chalk or pen work.
2. **`objects/flower-wide`'s night face is cross-eyed** *(shipped asset)*: the night fill painted
   ...
   notes + re-punch.
3. **`vehicles/garbage-tall`'s night fill drew a leaf on the trash can** *(shipped asset)*: the pen,
   ...
   first confirmed FILL-side invention inside the subject (every #5 invention came from the chalk).
...
4. **`vehicles/excavator-tall` ships noisy static around its night lines — and the halo auditor
   ...
   `space/ship-tall` 8.5 (#5's porthole page — its regen covers this too), excavator-tall 6.5,
   `shapes/rectangle-tall` 4.3, `objects/flower-wide` 4.0 (#2's page), `shapes/heart-tall` 3.2,
   ...
   wave gates on it.
5. **The chalk invents facial features — teeth on `farm/duck-wide`, whole faces on
...
6. **`farm/dog-wide` glows white at night — nose, eye whites, rump patch** *(shipped asset)*: the
...
7. **Chalk-whitened night eyes on `farm/horse-wide`, `farm/horse-tall`, `nature/caterpillar-wide`,
...
8. **`creatures/mermaid-tall`'s light eyes are giant solid-black orbs** *(shipped asset)*: the pen
   draws each pupil as solid ink filling nearly the whole eye (only two catchlight holes), so the
   light fill has nothing to paint and the face reads dead-eyed in light mode. This is #14's
   ...
   for #14's audit-driven wave. The one Tier 1 item needing pen work: normalize the pen
   (`gen:coloring-outlines:normalize`, seeded notes) + light-fill regen — mind #14's caveat (the
...
12. **Colored-shape invention is only audited, not gated (IDEAS #13)** *(gate blind spot)*: the
    ...
    its scope is the open background only — Tier 1 #5's invented teeth, apple faces, and porthole
    eyeballs, and Tier 1 #3's fill-side leaf, all sit *inside* the subject, where neither this audit
    nor any gate looks.
...
13. **Chalk whitening on solid-pen-eye pages is gate-blind** *(gate blind spot)* (proved by
    ...
    solid-pen-eye page: Tier 1 #7's horses are two of them). A solid pen pupil has no nested rings →
    ...
    prove the class is wider than its name: Tier 1 #6 (`farm/dog-wide`) shows it isn't eye-shaped —
    the chalk whitened a solid nose and even solidified an *outlined* rump patch — and Tier 1 #7's
    ...
    flag chalk ink the pen doesn't have (which would also catch Tier 1 #5's invented features).
14. **Light-mode eyes on accident-era pens are dead/solid** *(shipped asset; IDEAS #6 — the biggest
    ...
    2026-07-13 (its page fix is prioritized as Tier 1 #8). The root cause is the pen: a solid-ink
...
next mass regen happens. (The "nothing shipped looks wrong" framing died when the 2026-07-13 review
found the now-resolved green night ant: #16's gap had shipped a real defect, so the gate stays
here.)
...
16. **Light↔night and tall↔wide palette coherence are unenforced (IDEAS #8/#9)** *(gate blind
    ...
    *identity* color (the now-resolved night ant turned green). The hue-flip scorers and
    conditioning recipes in `ideas-exploration/idea-8`/`idea-9` were validated but not promoted;
    none of them checks subject-color plausibility, so promotion alone would not have caught the ant
    — the light fill's subject hue is the natural reference.
