...
1. **Dark-mode picker still shows inverted-pen thumbnails** *(shipped asset; IDEAS #19 — the only
   user-facing bug on this list)*: every page now has a chalk, but the app still derives thumbs from
   the pen, so a child taps an inverted-pen thumb and gets chalk art on canvas. The fix is fully
   offline: extend `gen-coloring-thumbs.mjs` to also resize `.chalk.webp`, plus a small theme-aware
   `thumbPath()` change in `books.ts`. The five 2026-07-13 fresh-outline pages ship
   `{page}.chalk.thumb.webp` files as the first batch (unused until the app change); the rest of the
   catalog is outstanding.
2. **The night mood bar's code default (≤ 100) is looser than the shipped catalog** *(gate blind
...
3. **Golden-set regression fixtures (IDEAS #23) and the light-asset sha256 manifest (IDEAS #25) are
...
4. **Promote the exploration auditors to first-class scripts** *(tooling gap)*: the invented-shape
   ...
   `scripts-info` descriptions (ADR-0019). Also the stepping stone to gating shape invention (#9).
5. **No per-page notes registry (IDEAS #10)** *(tooling gap)*: the 3.1 wave's customizations
...
9. **Colored-shape invention is only audited, not gated (IDEAS #13)** *(gate blind spot)*: the
   ...
   it's a generation-time gate (or at least a first-class audit script — #4 covers that half), a
...
10. **Chalk whitening on solid-pen-eye pages is gate-blind** *(gate blind spot)* (proved by
    ...
    eye-polarity gate (Stage 1.5 gate 4) passes vacuously, and the night eye judge is silent too
    (its chalk-white-nearby rule trusts the chalk). Only composite review catches it. A candidate
    scorer: chalk-ink fraction inside pen solid regions that sit at face positions. No ready patch —
    worth building before #11's burn-down, since that wave regenerates exactly these pages.
...
14. **Motif consistency across sibling pages is unenforced (IDEAS #2)** *(gate blind spot)*: the
    ...
    motifs in the notes registry (#5) so regens condition on them.
