...
1. **Promote the exploration auditors to first-class scripts** *(tooling gap)*: the invented-shape
   ...
   `scripts-info` descriptions (ADR-0019). Also the stepping stone to gating shape invention (#6).
2. **No per-page notes registry (IDEAS #10)** *(tooling gap)*: the 3.1 wave's customizations
...
3. **`judgeLightEyes` has no false-positive suppressions** *(gate blind spot)*. The IDEAS #12 fixes
   ...
   this before #8 — it de-noises the flat-eye list so the burn-down only spends API budget on real
   offenders.
4. **The orphan pages are still uncataloged (IDEAS #24)** *(shipped asset)*: `shapes/heart-wide` and
...
5. **Night subject/background contrast is unmeasured** *(gate blind spot)* (`shapes/circle-wide`
...
6. **Colored-shape invention is only audited, not gated (IDEAS #13)** *(gate blind spot)*: the
   ...
   it's a generation-time gate (or at least a first-class audit script — #1 covers that half), a
   light fill can ship an invented colored shape that keep/white/eye gates cannot see.

## Tier 3 — high impact but expensive

7. **Chalk whitening on solid-pen-eye pages is gate-blind** *(gate blind spot)* (proved by
   ...
   building before #8's burn-down, since that wave regenerates exactly these pages.
8. **Light-mode eyes on accident-era pens are dead/solid** *(shipped asset; IDEAS #6 — the biggest
   ...
   2026-07-13 fresh-outline pass) — though some are detector noise, not defects, which is why #3
   ...
   time.
9. **Style covers are still 2.5-era outputs** *(shipped asset)*. The 3.1 migration swapped the model
...
10. **Light↔night and tall↔wide palette coherence are unenforced (IDEAS #8/#9)** *(gate blind
...
11. **Motif consistency across sibling pages is unenforced (IDEAS #2)** *(gate blind spot)*: the
    ...
    motifs in the notes registry (#2) so regens condition on them.
