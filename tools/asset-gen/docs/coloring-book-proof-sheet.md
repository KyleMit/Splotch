...
| `--source git:<ref>` | render each page's committed assets **at `<ref>`** (before) directly above the current working-tree assets (after), so a regen reads as an old-vs-new pair in one sheet. Before-cell layers come straight from history via `git show <ref>:<path>`; where the shipped fills-only `.night.webp`/`.light.webp` predate a ref (pre-fork revisions) it falls back to the lined raw fill (`fill-src/…{light,night}.raw.webp`) and tags the tile `raw fill (pre-fork fallback)`. A ref or asset missing from a shallow clone degrades to a placeholder tile, never a crash. |
