...
// fill; anything lighter is a fill and kept —
// including legitimately dark fills (a ladybug's spots, a navy sky), because the
// mask keys off the LINE ART's darkness, never the fill's.
...
// The punched fill inherently costs more than the raw did: the binary alpha plane
...
export async function punchFill(rawPath) {
  ...
  // Line art luma at the fill's exact resolution → binary punch mask, interleaved
