...
//       },
//       "motifs": "…"                         cross-tool sibling-motif facts (e.g. the -wide has a
//                                             crescent moon) — printed for every tool, never applied
//     }
//   }
//
...
export function pageLevers(rel, tool) {
  ...
  const motifs = registry[page]?.motifs ?? registry['*']?.motifs ?? null;
  if (!wild && !own && !motifs) return null;
  return {
...
// cli / notes.json / default, plus the registry's retry recipe, review
// expectations, and sibling-motif facts. `settings` maps option name ->
// resolved value.
export function describeLevers({ rel, levers, fromRegistry, cliValues, settings }) {
  ...
  if (levers?.motifs) lines.push(`  motifs: ${short(levers.motifs)}`);
  return `${rel} levers:\n${lines.join('\n')}`;
}
