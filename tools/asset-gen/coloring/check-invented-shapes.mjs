...
//
//   npm run check:coloring-invented-shapes                    whole catalog
//   npm run check:coloring-invented-shapes -- space           one category
//   npm run check:coloring-invented-shapes -- space/ship-tall        both themes
//   npm run check:coloring-invented-shapes -- space/ship-tall.night  one theme
//   npm run check:coloring-invented-shapes -- --verbose  per-blob detail everywhere
//   npm run check:coloring-invented-shapes -- --overlay  dump detection overlays
...
import { COLORING_DIR, FILL_SRC_DIR, SAMPLES_DIR, toPosix } from '../lib/asset-paths.mjs';
...
for (const { fillPath, page, theme } of targets) {
  ...
  const source =
    theme === 'night'
      ? (await resolveNightLineArt(penPath)).source
      : await rasterizeLineArt(penPath);
...
if (flaggedPages) {
  console.log(
    'A flagged blob is paint with no source counterpart — confirm against the line art, then regenerate the fill (gen:coloring-fills / gen-night-fills) and re-punch. Washes are info only.'
  );
  process.exitCode = 1;
}
