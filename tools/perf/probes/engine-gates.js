...
// the same undo scenarios as `npm run perf:web:undo`, but on the real device
...
//   `npm run perf:analyze:web-inspector -- <export>.json` (the Web Inspector export is a
//   different, mark-only/ring-buffered format — NOT the Chrome-trace
//   perf:analyze:chrome). Override the volume with window.__perfStrokes /
...
(async () => {
  ...
  // Keys mirror the `--scenarios=` keys of `npm run perf:web:undo`, so a row found
  // hot here names the desktop scenario that reproduces it;
  // tools/perf/tests/engine-gates-driver.test.mjs fails if the two drift apart.
  const SCENARIOS = [
    {
      key: 'long-squiggles',
  ...
  if (TIMELINE) {
    console.log(
      'TIMELINE mode: stop the recording now and export it (Timelines tab → ' +
        'export icon), then: npm run perf:analyze:web-inspector -- <export>.json — NOT ' +
        'perf:analyze:chrome, which reads a Chrome trace. These milliseconds are the ' +
