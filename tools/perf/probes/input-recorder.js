...
// serves two jobs:
//
//   • Perf replay (`npm run perf:web:replay`): the canvas-targeted events give the
...
//   4. [Mac]  npm run perf:web:replay -- --recording=perf-profiles/recordings/my-session.json
//
...
(() => {
  ...
  const recPointer = (e) => {
    ...
    // which is what perf:web:replay fires. Capture retargets events, so a captured
