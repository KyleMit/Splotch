...
for (const [rel, w, h] of pages) {
  ...
  await sharp(chalk)
    ...
    .resize(resize)
    .webp({ quality: 85 })
    .toFile(join(OUT, `${name}.chalk-display.webp`));
  ...
  await sharp(lightRaw)
    .resize(resize)
    .webp({ quality: 85 })
    .toFile(join(OUT, `${name}.light-combined.webp`));
  ...
  await sharp(night)
    .resize(resize)
    .webp({ quality: 85 })
    .toFile(join(OUT, `${name}.night-combined.webp`));
