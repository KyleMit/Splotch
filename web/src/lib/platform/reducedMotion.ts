...
// CSS keys every reduced treatment off `:root[data-reduce-motion]` and never
// off `@media (prefers-reduced-motion)` (reducedMotionCss.test.ts enforces it),
// so JS and CSS read one source and an explicit 'full' can override the OS.
