...
import {
  ...
  envState,
  freshTokens,
  freshTokensOutsideDev,
  freshTokensWithBlobs,
  freshTokensWithEmptyBlobs,
  freshTokensWithFailingBlobs,
  freshTokensWithSeedRace,
  seedBlobsOutsideDev,
  storeFor,
  usageGrantKey,
...
describe('stale-empty seed races', () => {
  ...
  it('confirms the current list when a transient reread failure precedes success', async () => {
    const store = await seedBlobsOutsideDev('legacy', ['current']);
