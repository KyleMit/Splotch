<script lang="ts">
  ...
  import { closeColorPicker } from '$lib/state/ui.svelte';
...
<style>
  ...
  /* ── Responsive trimming (ADR-0048) ──────────────────────────────────────
     ...
     These stay MEDIA queries on purpose: the picker is a full-screen modal,
     so its room is the viewport (via the dialog's 90vw/90vh caps) — the
     /components catalog demos it inside an <iframe> for the same reason. */
