<script lang="ts">
  ...
  // Each grained specimen names its own baked tile rather than reading
  // --paper-texture, for the same reason the colors come from `themes`: the
  // live variable only ever resolves to the active theme, which would paint
  // the light tile onto the dark chip.
  const paperSpecimens = [
    { label: 'light paper', value: themes.light.paper, grain: themes.light.paperTexture },
    { label: 'dark paper', value: themes.dark.paper, grain: themes.dark.paperTexture },
    { label: 'light margin', value: themes.light.paperMargin, grain: null },
    { label: 'dark margin', value: themes.dark.paperMargin, grain: null },
  ];
</script>
...
{#if group === 'materials'}
  ...
  <section id="paper" data-sg-section>
    <h3>Paper</h3>
    <p>
      The canvas is warm off-white paper under a handmade-paper grain, tiled. Each theme ships its
      own <em>opaque</em> tile with the grain already composited onto its paper color (<code
        >--paper-texture</code
      >, generated by <code>npm run gen:paper-texture</code>) so the fill costs no per-pixel blend.
        ...
        <div class="paper-card">
          <div
            ...
            style:background={specimen.grain
              ? `${specimen.value} ${specimen.grain} repeat`
              : specimen.value}
          ></div>
