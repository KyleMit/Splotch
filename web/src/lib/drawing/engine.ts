...
// --- Crayon paper-tooth seed (ADR-0065) -------------------------------------
// Each stroke (one finger's continuous gesture) gets a seed that phases crayon's
// paper-tooth holes. Within a stroke every op shares it, so overlapping per-frame
// ops punch identical holes (idempotent — no joint beads); a LATER stroke gets a
// different seed, so its holes fall where the earlier wax's gaps were and the
// overlap fills toward solid (coverage buildup at a constant hue). The seed is
// stored on each op, so replay reproduces the same grain (ADR-0033). A plain
// counter is enough — the crayon renderer hashes it, so consecutive values
// decorrelate; only crayon reads it, other brushes ignore it.
let strokeSeedCounter = 0;
function nextStrokeSeed(): number {
  strokeSeedCounter = (strokeSeedCounter + 1) | 0;
  return strokeSeedCounter;
}
...
interface PointerState {
  ...
  erase: boolean;
  brush: BrushKind;
  // Paper-tooth phase for this stroke's crayon grain (see nextStrokeSeed).
  seed: number;
  lastTime: number;
