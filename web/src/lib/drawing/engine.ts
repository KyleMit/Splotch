...
// TRIAL T7: no mid-stroke stamps — read+glaze once per stroke.
...
function strokeCrayonSegments(ps: PointerState, points: Point[], moveCount = 1) {
  let batch: Point[] = [];
  let didSplit = false;
  // TRIAL T7: splits neutralized with the checkpoint (cadence trial).
  const T7_NO_SPLIT = true;
  for (const p of points) {
    if (!T7_NO_SPLIT && ps.passTracker!.advance(p) === 'split') {
