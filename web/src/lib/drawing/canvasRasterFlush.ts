import { isAndroidBrowser } from '$lib/platform';
...
// channel already holds. Measured on Android Chrome only, so every other
// browser keeps its unflushed path.
...
export function withCanvasRasterFlush(work: () => void) {
  const context = isAndroidBrowser() ? channelFlushContext() : null;
  work();
  if (!context) return;
