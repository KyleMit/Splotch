...
// decode-pending skip.
//
// While the colour mix is armed for the target (colorMix, ADR-0065) and the op
// overlaps pre-stroke ink, each pass strokes with a PRE-MIXED no-repeat
// pattern of the op's bounds instead of the repeating tile: same binary tooth
// alpha, RGB already pulled toward the under-ink snapshot per texel. The op
// deposits its final bytes immediately — the blend is live under the
// fingertip and nothing settles at lift — and because the mixed value is a
// pure function of the texel (fixed snapshot, stored seed), overlapping live
// ops and their fewer simplified replay ops still resolve to identical
// pixels. The JS tile lookup needs an identity transform; under a
// rotation-locked paper view ops fall back to the plain tile (unmixed), and
// the recorded `mixedUnder` flag keeps every rebuild consistent with that.
// Boxes up to this edge pre-mix their paint (path A, every live per-frame
// op); bigger boxes — replayed simplified ops spanning whole strokes — render
// the plain tile and then fix up their ink-overlap rect in place (path B),
// whose cost tracks the overlap instead of the op. The gate is a pure
// function of the op's stored geometry, so every rebuild picks the same path
// for the same op.
const MIX_PREMIX_MAX = 192;
...
function paintCrayon(
  ...
  const m = typeof target.getTransform === 'function' ? target.getTransform() : null;
  const identity =
    !m || (m.a === 1 && m.b === 0 && m.c === 0 && m.d === 1 && m.e === 0 && m.f === 0);
  const premix = !!box && identity && box.w <= MIX_PREMIX_MAX && box.h <= MIX_PREMIX_MAX;
  for (let i = 0; i < passes.length; i++) {
    const pattern =
      (premix ? crayonMixedPatternFor(target, box!, op.color, seed, i) : null) ??
      crayonPatternFor(target, op.color, seed, i);
    if (!pattern) continue;
    paintOpShape(target, op, pattern, passes[i].widthScale);
  }
  if (!box || !identity || premix) return;
  // Path B: re-stroke the op's tooth into a rect-local scratch, mix the
  // deposits in place, and blit them over the pure render. Outside the rect
  // there is no under-ink, so mixed == pure and the blit boundary is seamless.
  const rect = crayonMixOverlapRect(target, box);
  if (!rect) return;
  const scratch = crayonMixScratchFor(rect.w, rect.h);
  if (!scratch) return;
  const s = scratch.ctx;
  s.save();
  s.setTransform(1, 0, 0, 1, 0, 0);
  s.clearRect(0, 0, rect.w, rect.h);
  s.beginPath();
  s.rect(0, 0, rect.w, rect.h);
  s.clip();
  s.translate(-rect.x, -rect.y);
  for (let i = 0; i < passes.length; i++) {
    const pattern = crayonPatternFor(s, op.color, seed, i);
    if (!pattern) continue;
    paintOpShape(s, op, pattern, passes[i].widthScale);
  }
  s.restore();
  const img = s.getImageData(0, 0, rect.w, rect.h);
  mixDepositsInPlace(img, rect.x, rect.y);
  s.putImageData(img, 0, 0);
  target.save();
  target.setTransform(1, 0, 0, 1, 0, 0);
  target.drawImage(scratch.canvas, 0, 0, rect.w, rect.h, rect.x, rect.y, rect.w, rect.h);
  target.restore();
}
