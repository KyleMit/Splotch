...
describe('maxActionButtonScale', () => {
  ...
  it('caps below 100% on a small landscape phone', () => {
    ...
    // n=7: (667 − 156 − 64 − 136) / 7 = 44.43px per button → 74% of the 60px base.
  ...
  it('never drops below the slider minimum', () => {
    ...
    // n=7: 30.3px per button would be 50% — clamped to the static minimum.
  ...
  it('uses the vertical budget and 55px base in portrait', () => {
    ...
    layout.viewportHeight = 536;
    // n=7: (536 − 76 − 8 − 136) / 7 = 45.14px per button → 82% of the 55px base.
  ...
  it('subtracts safe-area insets from the budget', () => {
    layout.viewportWidth = 723;
    layout.viewportHeight = 375;
    Object.assign(layout.safeArea, { left: 30, right: 30 });
    // n=7: 60px of insets off the budget: (723 − 156 − 64 − 60 − 136) / 7 = 43.86px → 73%.
