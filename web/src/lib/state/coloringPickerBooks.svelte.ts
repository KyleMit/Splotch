...
import { availableColoringBooks, coloringPackState } from './coloringPacks.svelte';
...
// answer an open still takes is the installed-book scan it beat (`scanSettled`)
// — a returning child's books, not a download — and that lands without moving
// anything: the open shows the book list with a place reserved for every
// catalog book, and the scan's covers fill the places after the starter book,
// which comes first in the catalog.
export function createColoringPickerBooks(platform: BookPlatform): ColoringPickerBooks {
  ...
  $effect.pre(() => {
    if (!takesInstalledScan || !coloringPackState.scanSettled) return;
  ...
  return {
    ...
    holdForOpen() {
      takesInstalledScan = !coloringPackState.scanSettled;
