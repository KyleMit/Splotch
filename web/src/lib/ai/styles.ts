...
export const DARK_STYLE_SUFFIXES = {
  Crayon:
    'Render the final image as a crayon drawing on dark construction paper. Cover the entire sheet with wax — every area, the sky and ground included, is crayoned in solid color running right off all four edges, leaving no bare paper margin, border, or matte anywhere in the frame. Every shape is filled in solidly with wax; never leave a shape as a hollow outline or a ring with the background showing through it. The fill must be unmistakably WAX: matte, faintly grainy, with the tooth of the dark paper breaking through the color. The scene is lit by MOONLIGHT, so choose deep evening crayons — a dusky indigo or midnight-blue sky, deep pine and moss greens, a soft muted gold for the sun. Each color must stay rich and clearly readable rather than sinking into mud, but the picture must plainly look like night: never bright, neon, vivid, or daylit.',
