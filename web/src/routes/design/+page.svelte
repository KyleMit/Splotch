<script lang="ts">
  ...
  let siteHeader = $state<HTMLElement>();
...
<style>
  ...
  .page {
    /* Where a jumped-to heading parks: clear of the sticky header, plus air.
       ...
       agree with this page's chrome, and this route is what owns that. It, its
       narrow-width override, and --spy-shallowest-line all derive from
       StyleguideHeader's --header-row-height; design-navigation.spec.ts
       measures the live header against section arrival. */
    --heading-park: 96px;
  ...
  @media (min-width: 980px) {
    .page :global(.site-header .header-toc) {
      display: none;
    }
