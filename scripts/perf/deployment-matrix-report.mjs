...
function normalizeDrawingRun(source) {
...
function normalizeDrawing(sources = {}) {
  return Object.fromEntries(
    BRUSHES.map((brush) => {
      const runs = (sources[brush] ?? []).map(normalizeDrawingRun);
      return [brush, { aggregate: aggregateDrawingRuns(runs), runs }];
    })
  );
}

function normalizeUndo(source) {
...
function normalizeActions(source) {
  if (!source) return null;
  const profile = readJson(sourcePath(source));
  const results = profile.summaries.map((summary) => ({
    label: summary.label,
    count: summary.count,
  ...
  return {
    ...
    worst: {
      firstFrameP95: round(Math.max(...results.map((result) => result.firstFrame.p95))),
      postActionFrameP95: round(Math.max(...results.map((result) => result.postActionFrames.p95))),
      postActionFrameMax: round(Math.max(...results.map((result) => result.postActionFrames.max))),
    },
    results,
  };
}
...
function normalizeTarget(target) {
  ...
  return {
    ...shared,
    drawing: normalizeDrawing(target.drawing),
    undo: normalizeUndo(target.undoSource),
    actions: normalizeActions(target.actionsSource),
  };
}
...
function normalizeMatrix(manifest) {
  return {
    ...
    targets: manifest.targets.map(normalizeTarget),
  };
}
...
function renderReport(matrix) {
  ...
  const header = masthead({
    ...
    tagline:
      'One current-build snapshot across physical devices, simulators, browsers, and native shells. Every mark is scaled against the same acceptance gates.',
    home: '../../index.html',
...
<main><div class="shell">
  <p class="matrix-intro">The tiled renderer at <code>${esc(matrix.productCommit)}</code> is the product under test. Physical-iPad web is the calibrated release gate; simulator, emulator, native-shell, and desktop rows reveal regressions and platform differences but do not substitute for trusted physical input.</p>
  <div class="matrix-links"><a class="matrix-link" href="data.json">Normalized results JSON</a><a class="matrix-link" href="index.md">Detailed narrative and full table</a><a class="matrix-link" href="sources.json">Source manifest</a></div>
...
export { normalizeMatrix, renderReport };
