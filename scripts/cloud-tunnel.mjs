...
if (!TUNNEL_AUTH) {
	die('TUNNEL_AUTH is not set. It must match the Fly relay AUTH secret — set it in the\n' +
...
try {
	...
	track('chisel', spawn(chisel, [
		'client', '--auth', TUNNEL_AUTH, '--keepalive', '25s',
		PUBLIC_URL, `R:127.0.0.1:${REMOTE_PORT}:localhost:${PORT}`
	], { stdio: ['ignore', 'inherit', 'inherit'] }));
	await waitFor('public URL', async () => {
		try {
