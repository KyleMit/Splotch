// The shared "chrome" for every page under /scrapbook — the crayon masthead,
...
//
// This is the single source of truth for the scrapbook look. The Node generators
// that live under scripts/ import it directly (scrapbook-index, the icons sheet,
...
// Each theme's palette, declared once and emitted into both of the selectors that
// carry it: the OS preference (`@media (prefers-color-scheme)`) and the explicit
// toggle (`:root[data-theme=…]`). Structural tokens that don't vary by theme
// (radii, shell width, font, the crayon hues) stay in the `:root` block below.
const LIGHT_TOKENS = {
  paper: '#f5f3ee',
  'paper-2': '#efece5',
  card: '#fdfbf6',
