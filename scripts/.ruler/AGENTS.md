...
* Every script must run on macOS and Linux. Use plain Node `.mjs` for shared automation; scripts
...
* Shared helpers live in `scripts/lib/` — `android.mjs` resolves the SDK and AVD locations per
  ...
  vite dev/preview server in a detached process group so `stop()` cannot orphan the vite grandchild;
...
* TypeScript-flavored scripts run via `node --experimental-strip-types` (see the `check:assets` npm
  script).
* Inline POSIX environment assignments are valid in npm scripts.
