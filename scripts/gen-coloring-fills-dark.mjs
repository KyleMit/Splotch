...
- EYES must look bright, sparkly, and ALIVE — never a dark hole or a hollow socket. Paint each eye as a real cartoon eye with THREE distinct tones, brightest to darkest: (1) a generous LIGHT off-white sclera that clearly shows around the iris; (2) an IRIS in a clear, LUMINOUS jewel colour — a moonlit bright blue, teal, amber, or brown that GLOWS and pops against the dark scene (like a lit accent), NOT a dark navy, not near-black, and never the same dark tone as the night background; (3) only a SMALL dark PUPIL at the very centre, plus a tiny bright catchlight glint. On a big cartoon eye the iris is large, so if the iris is coloured dark the whole eye becomes a black hole — keep the iris a bright, saturated, mid-light colour so the eye reads as a sparkling eye, not a sunken socket. NEVER fill a whole eye with one dark colour. Eye-whites and the catchlight may take a near-white tint.
