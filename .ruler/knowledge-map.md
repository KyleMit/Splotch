...
| `docs/AUDIT-COMMENTS/`   | Generated archive of every per-finding comment the `burn-down-audits` runs posted to their PRs (\#535–\#589), split into one document per finding category (duplication, naming, dead code, …) — the durable record of what each fix did, what the adversarial reviewer caught, and why findings were dropped, since `docs/AUDIT.md` is drained as they land. Start at its `README.md` for the category index and run table. The PRs stay canonical; regenerate rather than hand-edit |
