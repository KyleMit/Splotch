...
| `fix-performance-matrix` | Freshly inventory the matrix, fix current scoreable reds, and ship causal clusters as reviewed stacked PRs |
| `lighthouse-audit`       | **Page-load** performance / Core Web Vitals on a throttled device; also an audit producer                  |

`run-performance-matrix` is the capture and refresh workflow. `fix-performance-matrix` consumes that
evidence and owns the sustained remediation campaign through zero current, scoreable reds or a
user-requested merge-ready wrap-up.
