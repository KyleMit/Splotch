...
1. Edit `docs/AUDIT.md` in place — remove the findings that don't hold up, enrich the ones that do.
   ...
   `#### Verification` inside (see `.ruler/skills/audit-conventions/SKILL.md`); preserve each
   `## Source:` section and the file header.
2. Add one row to `docs/AUDIT-LOG.md` for this run per `.ruler/skills/audit-conventions/SKILL.md` §2
   (date · `vet-audits` · one-line prune summary — what you kept/enriched vs removed).
