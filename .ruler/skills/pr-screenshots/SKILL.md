...
2. Reference them in the PR body by raw URL — GitHub resolves it **server-side**, so it renders
   ...
   through the agent proxy, whose CONNECT handshake always returns `HTTP/1.1 200 Connection
   Established`, masking the real origin status so a 404'd URL still reads `200`.)
