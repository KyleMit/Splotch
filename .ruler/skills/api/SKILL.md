...
A mutation on an instance where Blobs is configured but the read failed returns
`503 { ok: false, error }` and changes nothing. The in-memory fallback only absorbs writes where
Blobs is absent altogether (local dev); writing into it during an outage would report a token
add/revoke that the durable list never saw and that vanishes on recovery. The status per failure
reason is `MUTATION_FAILURE_STATUS` in `web/src/lib/server/tokens.ts`, shared with the `/admin` form
action so both front doors answer the same failure identically.
