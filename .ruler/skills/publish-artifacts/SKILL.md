...
description: Attach the built .aab/.ipa to the GitHub Release for a version, verifying each artifact's embedded version and source commit first. Use after the cut-release and build skills, or whenever a GitHub Release is missing its binaries or has the wrong ones attached.
---
...
the version and source commit is what makes the wrong one impossible (ADR-0077).
...
2. **Verify before uploading.** Run `npm run release:publish -- --dry-run`. This reads the version
   and `build-provenance.json` out of each artifact's own bytes (the `.aab`'s protobuf
   `AndroidManifest.xml` and packaged web assets, the `.ipa`'s `Info.plist` and packaged web
   assets). It compares versionName/versionCode with `releases/<version>.md` and the embedded full
   commit SHA with the `v<version>` tag commit. Report exactly what it found before doing anything
   else.

3. **Handle the outcomes.** Do not work around a refusal — every one of them means the upload would
   have been wrong:

   * **A stale artifact** (version or source-commit mismatch) — the build output directory holds an
     artifact from a different release state, even when its version number happens to match. Tell
     the user which artifact and which embedded version/commit was found, and that the fix is to run
     `build` from the release tag. A missing provenance manifest means the artifact predates this
     guard and must also be rebuilt. Never delete the stale file and upload the other one silently,
     and never pass a flag to skip the check.
...
5. **Report** the release URL, which artifacts were attached with the version and commit each
   carries, and any platform deliberately skipped. Remind the user that attaching to the GitHub
   Release is **not** the store submission — uploading to the Play Console and Transporter is still
   manual, with the "What's new" text at
