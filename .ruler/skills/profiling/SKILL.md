...
| `npm run perf:ipad:actions`                   | discrete UI actions through the same Appium path — drawer, palette, brushes, stroke width, responsive Parent Center, themes, coloring, screenshot, undo, drag-to-clear, and rotation. It supports mobile web or a native Capacitor WebView, repeats three times, records rAF inside the page, and fails the 20 ms P95 / 33.5 ms first/worst-frame gates. `--actions=` focuses one family; `--report-only` ranks a broken sweep without stopping                                          | action-local raw samples + grouped first-frame/readiness/frame P95/max verdicts (`actions.json`)                                                 |
