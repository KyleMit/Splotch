...
credential helper; no token belongs in a plist, environment variable, remote URL, or log. Report
commits and pushes happen in a freshly fetched disposable worktree, so a failed publication cannot
advance or dirty the control clone.
...
three stops publication. Before the build, `ideviceinfo` also verifies that the selected UDID's
measured `ProductType` matches `--device-model`. Successful reports go through
`npm run scrapbook:publish` to `scrapbook/performance/ipad-release-rig/` and appear on Pages.

The UDID remains only in the mode-0600 launchd configuration and ephemeral raw captures; the
personal device name remains only in raw captures. The normalized JSON, HTML, and scrapbook output
replace them with a neutral rig label and retain only the hardware model and iPadOS version.
Persistent launchd output uses the same neutral label and redacts the UDID from echoed command
lines.
