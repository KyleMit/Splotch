...
| `drawing/crayonTexture.ts`      | The crayon brush's paper-tooth paint (ADR-0065): a fixed binary tooth mask (deterministic multi-octave noise) tinted per colour and phase-offset per stroke `seed`, returned as a paper-anchored `CanvasPattern` that `renderOp` strokes crayon ops with. Opaque + seed-driven, so overlap is idempotent, replay is bit-identical, and a second same-colour pass builds up wax at constant hue.                                           |
