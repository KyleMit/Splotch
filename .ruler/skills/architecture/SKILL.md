...
| Path                            | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `drawing/engine.ts`             | Imperative canvas engine — the facade and orchestrator (ADR-0004). Owns the `<canvas>`, the paper coordinate space, all pointer tracking (with the WebKit pen quirks and edge-swipe guards), and the whole public API; delegates ops/undo/export to the sibling modules below. Components connect via callbacks (`onDrawSound`, `onUndoStateChange`, etc.) and direct calls (`setColor`, `setStrokeWidth`, `clearCanvas`).                                                                                                                          |
| `drawing/strokeOps.ts`          | The engine's op vocabulary (`StrokeOp`, `StrokeGroupCommand`) and the single `renderOp()` renderer every surface shares, so live drawing and the commit fold / export repaint are bit-identical (ADR-0066).                                                                                                                                                                                                                                                                                                                                         |
| `drawing/crayonBrush.ts`        | The crayon brush's paint source (ADR-0065): a deterministic tileable paper-tooth field and per-(colour,pass) repeating `CanvasPattern`, phase-shifted by a stored per-stroke seed so overlapping same-colour strokes build up (fill tooth) at a constant hue. Owns the tunable knobs + the dev A/B seam. Closed passes commit as live-captured rasters the fold blits (ADR-0068).                                                                                                                                                                   |
| `drawing/undoHistory.ts`        | Undo history (ADR-0066): the committed "paper" raster + a depth-20 stack of pre-stroke snapshots (2 hot rasters, deeper entries as lossless encoded blobs), so undo/resize/remount/export are blits. Deliberately module-level so the drawing survives the engine's teardown/init across client-side navigation (ADR-0004).                                                                                                                                                                                                                         |
...
| `boot/`                         | The drawing route's named boot steps, called in order from `routes/+page.svelte`'s `onMount`: `hydratePersistedState()` (API key, save folder, durable-storage recovery), then `mountBootHiddenOverlays()` (the idle overlay pump, ADR-0049), `installContextMenuGuard()`, `installWakeLock()`, and `initWebOnlyServices()` (PWA updates + install prompt, web-only). Those last four return teardowns the route collects and runs on unmount — page-lifecycle-scoped imperative wiring, as distinct from the self-initializing stores in `state/`. |
| `components/`                   | Svelte UI components. Each component owns its scoped styles.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `components/page/`              | `PageShell` + `RuleLabel` — the shared chrome of the standalone, link-shareable pages (`/android-beta`, `/feedback`): ground, centered sheet, masthead (back link + `CrayonStrip` + wordmark), hero, and the `--page-*` palette everything nested inside reads. The palette defaults to the themed app tokens; a page that must stay light-only pins them on the forwarded `class`.                                                                                                                                                                 |
| `components/report/`            | `ReportFields` — the feedback form's field set (kind picker, message, public-posting note, device-info opt-in + preview, honeypot), shared by the Parent Center's `ReportForm` (posts JSON to `/api/report`) and the `/feedback` page (posts to a form action). Every control carries the `name` the form action reads.                                                                                                                                                                                                                             |
| `actionButtonLayout.ts`         | Actions Panel button-row geometry shared by two consumers that must agree: `ActionsPanel.svelte` caps the rendered button size and the Parent Center Button Size slider caps its range (`maxActionButtonScale`). Plus `publishActionPanelState`, the DOM writer that stamps the panel's persisted UI state (`--action-btn-scale`, `data-drawer-open`, `data-off-*`, `data-brush`) onto `<html>` for CSS. Owns no `$state` — reads the state modules, driven by a reactive caller (like `theme.ts`).                                                 |
...
| `pwa/updates.ts`                | Registers the PWA service worker (deferred behind the stroke-count + idle gate on a first visit; immediately at idle on repeat visits) and checks for updates, auto-applying them (with a reload) only while the canvas is blank; otherwise the update activates on next launch.                                                                                                                                                                                                                                                                    |
...
| `/android-beta`           | SSG    | Google Play closed-test sign-up instructions. Light-only, `noindex`; built on `PageShell`. `prerender = !__IS_CAPACITOR__` keeps it out of the native export.                                                                                   |
| `/feedback`               | SSR    | Standalone bug/idea form — the link-shareable twin of the Parent Center's Send Feedback section, built on `PageShell` with the same `ReportFields`. `prerender = false` (it has a form action), so it is web-only and works without JavaScript. |
