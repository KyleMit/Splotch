...
| `drawing/crayonBrush.ts`        | The crayon pen's waxy-paper texture (ADR-0065): a deterministic two-octave tooth — a per-stroke-phased colour deposit (alpha-only, so same-colour overlap builds up without shifting hue) plus a paper-anchored `destination-out` weave carve (permanent tooth + frayed edge). `renderOp()` branches here for `brush: 'crayon'` ops.                                                                                                      |
...
| `state/brush.svelte.ts`         | The pen's texture style (`'crayon'` default vs. `'marker'` flat, ADR-0065). `DrawingCanvas` pushes it into the engine via `setBrush()`; the `marker` is retained as an A/B variant.                                                                                                                                                                                                                                                       |
