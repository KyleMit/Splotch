...
| `drawing/crayonBrush.ts`        | The pen's waxy crayon texture (ADR-0065): fixed-seed paper-tooth alpha tiles painted as a paper-anchored `repeat` `CanvasPattern`. Hard-alpha => idempotent replay; a per-stroke `seed` picks the tile so overlapping same-colour strokes build up. Default brush; `flat` stays A/B-able via the `/dev/engine` seam.                                                                                                                      |
