...
| `drawing/crayonTexture.ts`      | The crayon brush's paper-tooth texture (ADR-0065): a deterministic seeded-fBm alpha tile, colourised per op colour and tiled in paper space, that `renderOp()` strokes a crayon op through — so overlapping same-colour strokes build up (fill the tooth, constant hue) via `source-over` alpha accumulation. Dev-A/B variants via `setCrayonParams`.                                                                                     |
