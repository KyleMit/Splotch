...
Both stores accept a single orientation per device type. Splotch is fully responsive, so the assets
use the most natural fit:
...
The 7" and 10" Play tablet images are intentionally identical (same 1920×1080 capture) — Play allows
reusing them, and it satisfies both size specs.
...
* **Third-party IP kept out.** Every shipped coloring book is original work — no trademarked
  characters or franchises, in the packs themselves or in `web/static/coloring/COLORING-BOOK.md`,
  which is served from `static/` and so is readable inside both native bundles. Keep it that way: a
  branded pack is an IP-rejection risk on both stores, and worse in a children's-audience app.
