...
its rendered GitHub blob view). A registry key is a folder path, so one page nested inside a
collection can earn its own card — the two performance explainers under `performance/` sit in the
Explainers group rather than as chips on the Performance-matrix card. `npm run scrapbook:check` (run
in CI) fails if any collection dir has no such reachable entry page, so every published collection
