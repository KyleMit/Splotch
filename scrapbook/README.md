...
fallback for any unknown type that lists its report pages (`.html` on-site, `.md` linked to its
rendered GitHub blob view). `npm run scrapbook:check` (run in CI) fails if any collection dir has no
such reachable entry page, so the index's collection count always matches the cards it shows.
