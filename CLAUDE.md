...
| `create-handoff` / `resume-handoff`     | pausing in-flight work for a later session (`create-handoff` — a copyable continuation prompt by default, a `docs/handoff/` packet on request), or picking it back up (`resume-handoff`)                                                                                                           |
...
| `docs/ORIENTATION.md`            | Which orientation controls ship to which device and target, what each does to the screen, and why. The device x target x rotation-setting matrix with its evidence level, the detection predicates (and the two places the OS rotation setting cannot be read at all), the APIs in play, and the E2E helpers a web orientation spec needs. Read before changing the Orientation picker, a rotation lock, or any branch deciding whether the app may turn the screen   |
