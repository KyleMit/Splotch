...
* The npm `android:*` scripts run the Gradle wrapper via `scripts/gradle.mjs`, which picks `gradlew.bat`/`./gradlew` per platform (ADR-0017), so they work the same on macOS, Linux, and Windows. `npx cap run android` also works.
