...
* The npm `android:*` scripts run the Gradle wrapper via `tools/mobile/android/run-gradle.mjs`,
  which resolves `./gradlew` to an absolute path and runs it from `android/` (ADR-0017), so they
  work the same on macOS and Linux. `npx cap run android` also works.
