...
* The npm `android:*` scripts run the Gradle wrapper via `scripts/gradle.mjs`, which resolves
  ...
  macOS and Linux. `npx cap run android --flavor generic` also works.
* Android has `generic` and `play` distribution flavors (ADR-0103). Generic is the debug, emulator,
  sideload, and non-Play baseline. The canonical `android:bundle` release enables the Play
  asset-pack module and builds `playRelease`; `android:bundle:generic` is the signed HTTPS-only
  escape hatch. Asset-pack `src/main/assets/` trees are generated by the release script — never edit
  them.
* Release signing reads `android/keystore.properties` (git-ignored; template at
  `keystore.properties.example`). Values must not be quoted. Without the file, release variants
  produce unsigned AABs.
