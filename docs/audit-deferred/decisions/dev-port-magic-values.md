...
`web/netlify.toml`, `scripts/*.mjs` — deferred because the implementer failed to deliver a fix round
...
`scripts/lib/dev-ports.mjs` and rewrote every consumer (plus four new wrapper scripts) to import it.
...
* `scripts/tests/dev-ports.test.mjs` (8 tests, in `test:scripts`, in CI, passing) reads
...
What the guard deliberately leaves out is the `4173` preview port (`scripts/perf/serve.mjs`,
`scripts/perf/args.mjs`, `scripts/e2e-sweep.mjs`, `scripts/driver-smoke.mjs`,
`scripts/gen-large-image.mjs`, `scripts/store-shots.mjs`, `web/playwright.shared.ts`). Every one of
those scripts spawns its own preview server from the same constant it then connects to —
self-consistent within one file by construction, so no cross-file agreement is required for
correctness: two of them on different ports would both still work. `web/playwright.shared.ts` goes
further and resolves the port at runtime from `SPLOTCH_E2E_PORT` precisely so concurrent worktrees
pick *different* ports (the root concurrent-worktrees convention); a shared must-agree constant
would misstate that relationship. The `4173` literals in `scripts/tests/*.test.mjs` are the
convention's explicit exception ("tests deliberately excepted").

The draft patch no longer applies — 14 of its 15 files conflict at HEAD. Its structure is also
superseded: its `perf-serve.mjs` wrapper exists differently as `scripts/perf/serve.mjs` (with LAN
URL filtering), `playwright.shared.ts` gained the runtime port resolver, and its `dev-kill.mjs`
wrapper overlaps a separate still-open deferred finding about `dev:kill`'s bare `npx kill-port`,
...
2. **Re-land `scripts/lib/dev-ports.mjs` on top of the guard.** Pure indirection now: the guard must
...
* Objection 1 (netlify.toml needs a drift guard, not prose) — satisfied at HEAD in exactly the form
  demanded: `scripts/tests/dev-ports.test.mjs` reads both sides, in the pattern of
  `scripts/tests/android-config.test.mjs`.
