...
| 36 | `p1-duplication-book-page-ids.md`                 | Book id re-typed on every `page()` call                     | P1  | C13     | FIX — re-implement book() on HEAD       |
| 37 | `p2-design-tokens-spacing-font-px.md`             | Spacing/font sizes raw px while colors/radii use tokens     | P2  | C14     | OPTIONS — lean: scoped swaps            |
| 38 | `p4-design-tokens-brand-rgb-fallback.md`          | Hardcoded brand RGB fallback drifts from `--brand`          | P4  | C14     | FIX — derived --brand-rgb               |
...
| 41 | `p1-duplication-gemini-wrappers.md`               | Six near-identical Gemini `generateContent` wrappers        | P1  | C15     | FIX — rebase onto HEAD gemini.mjs       |
| 42 | `p2-duplication-asset-gen-constants.md`           | Centralize `MODEL`/`WEBP_QUALITY`/timeout constants         | P2  | C15     | OPTIONS — lean: rides on #41            |
| 43 | `p2-duplication-flood-fill.md`                    | Background flood-fill written twice in lib (+ once in bin)  | P2  | C15     | FIX — floodFromBorder predicate core    |
| 44 | `p3-complexity-score-composite-eyes.md`           | `scoreCompositeEyes` 100-line function w/ inline validator  | P3  | C15     | FIX — patch + erodeCross                |
| 45 | `p3-architecture-fail-in-paths.md`                | `fail()` lives in `paths.mjs`, unrelated to path resolution | P3  | C15     | FIX — move to cli.mjs                   |
| 46 | `p5-maintainability-median-luma.md`               | "Median" via `>>1` + luma definitions differ across modules | P5  | C15     | OPTIONS — lean: safe subset             |
