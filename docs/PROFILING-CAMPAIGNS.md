...
capture record. The restricted HTTPS front checks file existence in its own `web/build`; stage the
historical build there byte-for-byte in an isolated harness checkout before serving it, keeping the
front's path allowlist intact. A native app records its loaded entry separately and must be
identified from its installed bundle, never from the harness checkout. The 2026-09-02 raster-tier
and backdrop-blur A/Bs instead ran their `origin/main` arms from the historical worktree itself. Two
shell-level traps from the same night: the default shell is zsh, which does **not** word-split an
unquoted `$ARGS` variable — the runner received one giant argument and asked for `--device-id` —
spell the flags out or use `${=ARGS}`; and killing the `serve-profile-build` wrapper can leave its
`vite preview` child holding the port with a manifest whose chunks a later rebuild has replaced
(`manifest OK` fails on the entry fetch) — `lsof -nP -iTCP:<port> -sTCP:LISTEN` names the child, and
its cwd (`lsof -p <pid>`) says whether it is yours to stop.
