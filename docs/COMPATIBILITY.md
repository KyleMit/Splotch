...
| `createImageBitmap(blob)`                                               | `lib/drawing/undoHistory.ts` — deep-undo blob decode + K_LIVE re-inflation (ADR-0066); also `routes/dev/engine/+page.svelte` (dev harness)    | Chrome 50 / Firefox 42 / Safari 15                                                                                                                              | ✅ rejections handled                                                   | within floor; a failed re-inflate decode keeps the blob (the next deep undo retries it), a failed deep-undo decode loses that one restore — the paper chain logs and survives                               |
| `canvas.toBlob(cb, 'image/webp', 1)` (WebP encoder)                     | `lib/drawing/undoHistory.ts` — cold-snapshot encoding (ADR-0066)                                                                              | Chromium + Firefox 105+ encode lossless WebP at q=1; Safari (all versions, through 27) has no canvas WebP encoder and hands back the spec-mandated PNG fallback | ✅ `isValidColdSnapshotBlob` — an unexpected blob keeps the live raster | functional on every engine in the floor; Safari's PNG blobs are larger but still lossless, so restore stays byte-exact — the WebKit memory-budget variance is settled by the on-device perf gate (ADR-0066) |
