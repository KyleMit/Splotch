...
* `docs/DEPENDENCIES.md` still records `dev:kill` as an unpinned, runtime-fetched
  `npx kill-port 5173 8888` command; update the live dependency inventory to describe the new local
  helper.
* The repo-owned run-splotch workflow still prints and repeatedly recommends bare `npx kill-port`
  commands in `.ruler/skills/run-splotch/driver.mjs` and `SKILL.md`, preserving the same
  runtime-download and supply-chain defect for kept servers; replace those instructions with a
  local, locked mechanism and regenerate the mirrored skill outputs.
* `scripts/dev-kill.mjs` delegates to the best-effort `freePort()`, which only sends `SIGTERM` and
  neither waits for nor verifies listener exit; `npm run dev:kill` can therefore return success
  while a wedged or signal-handling process still occupies 5173/8888, unlike the forceful cleanup it
  replaces. Add bounded verification with escalation or otherwise guarantee the ports are clear
  before exiting successfully.
* The documented `--port` path maps arbitrary strings through `Number` without validation, so values
  such as `--port nope` or an out-of-range port silently run `lsof` with an invalid target and exit
  successfully without cleanup. Reject non-integer ports outside 1–65535 before calling
  `freePort()`.
* `listenerPids()` in `scripts/dev-kill.mjs` treats an `lsof` process that exits nonzero or is
  terminated without producing stdout as proof that the port is clear, because it checks only
  `result.error`. Detect operational failures (while preserving the normal no-match case) so
  `dev:kill` cannot exit successfully while leaving listeners running.

#### What was tried

1. Replaced the unpinned npx invocation with a local Node entry point that reuses established port
   cleanup for 5173 and 8888, and updated the drift guard to read that entry point’s ports.
2. Updated the dependency inventory and run-splotch source to use project-local cleanup, with
   `--port` support for kept servers, and added a regression guard. Ruler generated the Claude copy
   but this sandbox denied the `.agents` output; the outer driver must regenerate that mirror.
3. Made dev-port cleanup validate every requested port, wait for SIGTERM to release listeners,
   escalate survivors to SIGKILL, and fail if verification cannot prove the port is clear. Added
   focused coverage for invalid input, escalation, persistent listeners, and unavailable
   verification.
