...
The driver's gates were red at the final round: the Playwright spec(s) tests/palette-trim.spec.ts
are red.

Reviewer's unresolved objections:

* The three updated snapshots change only the unselected custom-swatch icon region, although this
  fix changes no icon rendering; restore these unrelated baseline changes instead of blessing
  platform/rendering noise.
* `palette-trim.spec.ts` only captures the initial light-theme palette and never triggers ring
  expansion or selects the custom swatch, so it does not cover animation alignment, the dark-theme
  seam, or the custom swatch’s resting ring. Add targeted coverage for those acceptance-critical
  states.

#### What was tried

1. Centralized the selection-ring width and seam as component-local custom properties, then reused
   them for both the resting shadow and confirmation animation so their outer edges remain aligned.
2. Updated the three stale macOS palette snapshot baselines to the established current rendering.
   The failed pixels were confined to the previously resized custom-swatch honeycomb; the selection
   ring already matched its baseline.
3. Restored the three unrelated macOS snapshot baselines. Added focused palette tests that select
   and inspect the animated ring geometry, verify the dark-theme seam matches the palette surface,
   and confirm a committed custom color retains the expected resting ring.

#### Draft implementation

The rolled-back draft is kept at
`docs/audit-deferred/p3-maintainability-the-4-5px-selection-ring-width-is-a-magic-number-repe.patch`
(3 commits). It passed the driver's type-check, unit-test and lint gates — the review is what it did
not pass — so it is a starting point rather than scrap. Apply with
`git apply docs/audit-deferred/p3-maintainability-the-4-5px-selection-ring-width-is-a-magic-number-repe.patch`.
