...
* **A view an overlay picks *at open time* can't be waited into — reopen it.** Retrying an assertion
  ...
  in `onOpen` and holds the books that open shows until it closes: a book that finishes downloading
  mid-open joins at the next open, so the picker never moves under a finger. The one answer an open
  still takes is the installed-book scan it beat (a manifest fetch plus a store scan), which fills
  slots the open reserved for every catalog book. So a picker opened once only the starter book is
  known shows its pages and *stays* there — how "no viewport draws a cover smaller than four
  columns" came to fail with the cover grid simply absent (issue \#936) — and an open that beat the
  scan with only some books seeded keeps its unfilled slots. The sentinel to retry the open against
  is the view you need, not the dialog: `openColoringBookGrid` (`tests/flows-harness.ts`) closes and
  reopens until an open shows the grid with no slot left, since each open re-reads the installed
  set. Prefer it over `openColoringDialog` in any spec that then reads a book cover, and seed the
  page with `gotoAppWithInstalledColoringBook`/`…AllColoringBooksInstalled` so there is a grid to
  land on — a spec that means to exercise the fresh-install view drives the dialog directly instead
  (`coloring-pack-download.spec.ts`). On the web, seeding is also what makes a boot count as a
  returning visit: a device with no pack storage downloads nothing until the child draws a few
  strokes or opens the picker, so a plain `gotoApp` spec that needs other books must engage first.
