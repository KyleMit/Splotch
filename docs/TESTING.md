...
* **A view an overlay picks *at open time* can't be waited into — reopen it.** Retrying an assertion
  ...
  in `onOpen` and holds the books that open shows until it closes: a book that becomes known
  mid-open, by download or by the installed-book scan (a manifest fetch plus a store scan), joins at
  the next open, so the picker never moves under a finger. So a picker opened while only the starter
  book is known shows its pages and *stays* there, which is how "no viewport draws a cover smaller
  than four columns" came to fail with the cover grid simply absent (issue \#936). The sentinel to
  retry the open against is the view you need, not the dialog: `openColoringBookGrid`
  ...
  dialog directly instead (`coloring-pack-download.spec.ts`). On the web, seeding is also what makes
  a boot count as a returning visit: a device with no pack storage downloads nothing until the child
  draws a few strokes or opens the picker, so a plain `gotoApp` spec that needs other books must
  engage first.
...
* **Pace a spec on the animation it depends on, never on a clock sized to it.** The lifting-halo
  spec in `drawing-motion.spec.ts` needs a lift that has started and not ended when reduced motion
  ...
  rather than timing the act. The halo lift-off specs in the same file release the stroke partway
  through a 120 ms grow-in, which no sleep can hit reliably, so they override `--halo-in-duration`
  to 5 s through a `:root:root` rule that outranks the component's scoped declaration — the lift
  keeps its real duration, and no slow frame can end the grow-in before the release. The override
  has to come from a stylesheet rather than an inline style: the halo does not exist until the press
  mounts it, so by the time a spec could reach the element a starved frame may already have finished
  the grow-in. Those specs also resolve their outcome on the element's *removal* as well as on its
  animation events, so the pre-fix product — which deleted the halo before the lift could start —
  fails the assertion instead of hanging on an event that never comes.
