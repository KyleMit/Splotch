...
On `onerror` (line 32) the promise resolves `null` and `paperTexturePromise` stays set forever, so
every later call short-circuits to the cached failure at line 25. The failure mode is realistic:
`warmPaperTexture()` is fired from the engine's idle warm, so a momentary connectivity blip at idle
time (PWA on a flaky tablet connection) means every export for the rest of the session silently
ships without the handmade-paper texture, even though the network recovered long before the child
ever tapped save. CLAUDE.md: "A memoized promise resets itself on rejection (see
`web/src/lib/idb.ts`) unless permanent failure is intended" — nothing here says permanent failure is
intended (contrast with the deliberate degrade comments elsewhere in this file, e.g. `folderSave.ts`
line 62).

The module also keeps two parallel caches (`paperTextureImage` + `paperTexturePromise`) where the
promise alone suffices — the sync fast path saves one microtask on a rare, already-async export
path.

#### Proposed solution

Reset the memo on failure so the next export retries:
...
While there, consider collapsing to the single-`paperTexturePromise` shape (drop
`paperTextureImage`), matching the `lazyIdbDatabase` pattern in `web/src/lib/idb.ts`. The in-flight
`paintPaperBackground` call still receives `null` for that one export — acceptable, it already
handles it.
