...
```
What are the core code domains / areas of the app right now.  List them and report each domain by lines of code that touch that domain

ignore binary assets.  Ignore large data files, but keep important config files (package.json etc.)

ignore agent files that are duplicates via ruler apply, and only include .md files that are part of .ruler or independently authored.

For anything over 3k LOC, publish the whole lines of code for the group, and then try to split it up into separate buckets (does not need to recurse if a paticular bucket is still over 3k)

yes, write to docs/CODE-MAP. md
```
