...
deltas, callback delay, and `hiddenOverruns`. Each sample also carries `lastPreActionFrame`, the
boundary row: the frame stamped before the action, in the same shape as a `postActionFrames` entry,
and `null` when no frame preceded the action. Under rAF-aligned input it is the frame that ran after
the action and rendered it. Nothing scores it or feeds it to `frameStamps`, and captures older than
ADR-0163's 2026-09 amendment lack the key. The scorer never reads the actual channel; the committed
corpus is held to a byte-identical re-derivation across the change. Why the scheduled channel keeps
the gate, and what would justify moving it, is ADR-0163's.
