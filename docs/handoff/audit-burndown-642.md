...
**Tracked in [#624](https://github.com/KyleMit/Splotch/issues/624)** — repro command, evidence, and
four falsified fixes live there, not here, so they outlive this packet. Nothing was changed in the
test harness; the attempted fixes were reverted.

**Fix it before the next long run.** `E2E_CMD` carries `--retries=1`, so it never reddens a gate —
it just costs wall-clock on every UI-touching finding while each implementer re-diagnoses a failure
it did not cause. The canary's first finding already burned effort proving it was pre-existing.
