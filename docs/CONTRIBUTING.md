...
`<dir>/.ruler/` directories); rare runner-specific skill variants live in `.ruler/agent-overrides/`.
Don't edit the generated files — edit the `.ruler/**` source, run `npm run ruler:apply`, and commit
the regenerated output; CI fails on drift (`npm run ruler:check`).
