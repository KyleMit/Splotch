...
* Data leaves the device only through an explicit feature: a **drawing image** sent to the AI
  ...
  days; a picture report also retains the generated output. A daily scheduled function purges it.
  Optional feedback device details are private. Coloring-pack requests contain only public static
  asset paths; they carry no drawing or child data.
