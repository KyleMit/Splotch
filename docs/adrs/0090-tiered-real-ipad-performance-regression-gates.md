...
post-action interval at most 33.5 ms (raised to 50 ms by
[ADR-0156](0156-physical-rows-gate-releases-advisory-rows-never-count.md), which sizes the max for a
single-capture cell). The 33.5 admitted two exact 60 Hz vsync intervals plus timer precision; the
