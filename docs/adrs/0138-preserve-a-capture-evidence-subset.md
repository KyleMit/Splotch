...
Promotion is explicit — `npm run perf:evidence:keep --corpus=<dir> --campaign=<name>` — never a
default write path. The tree holds curated evidence only, and a capture enters it because someone
