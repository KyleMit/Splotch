...
(`{page}-{tall,wide}.outline.webp`) shown as the overlay, and a flat-colored twin
...
- **−** The twin must exist for every page (`bookAssetPaths` now lists the
  `.light.webp` twins so `check-assets` enforces it and native keeps them).
...
- The lined twins ("raws") moved out of `web/static` into
  `tools/asset-gen/twin-src/{book}/{page}-{orient}.{light,night}.raw.webp` —
