...
- **Backing store:** `resizeCanvas()` sets `canvas.width/height = rect × renderScale`, and the virtual canvas's `squareSide` scales the same way.
...
- **Undo snapshots and virtual-canvas syncs:** inherit automatically because they copy `canvas.width/height`.
...
- **`renderScale` is fixed per session.** A mid-session DPR change (desktop browser zoom, dragging between monitors) is *not* tracked: handling it would require rescaling the virtual canvas and every undo snapshot in place. Reload picks up the new DPR.
...
- **-** 4× the pixels on every surface. This *was* dominated by the undo stack (10 full-canvas snapshots, ADR-0004): roughly ~44 MB on a typical phone and up to ~160 MB on a 10″ tablet, versus a quarter of that at 1×. **Superseded by ADR-0033:** undo now keeps a single baseline raster plus a tiny command log, so the stack's per-snapshot multiplier is gone and the remaining 4× cost is the live backing store + virtual canvas (two surfaces, not twelve).
- **-** 4× fill rate per stroke segment and 4× per-stroke copies (snapshot, virtual-canvas sync). Expected to be absorbed by the GPU post-perf-work, but **not yet verified on a real device** — the `chrome://inspect` profiling workflow in the mobile guide (`.claude/skills/mobile/android.md`) is the follow-up.
