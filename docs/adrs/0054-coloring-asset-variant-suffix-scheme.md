...
ADR-0045), and the magic-brush twins were dot-suffixed (`cat-tall.color.webp`
ADR-0043, `cat-tall.night.webp` ADR-0052), with raw twins in `twin-src/` adding
...
- **`.color` misnamed its role**: it is specifically the *light-theme* twin,
...
web/static/coloring/{book}/{name}.light.webp     light magic-brush twin (fills-only)
web/static/coloring/{book}/{name}.night.webp     dark magic-brush twin (fills-only)
tools/asset-gen/twin-src/{book}/{name}.{light,night}.raw.webp   raw (lined) twins
```
...
- `lib/punch-twin.mjs` derives the shipped twin path from a raw by stripping
...
- CLI page arguments stay suffix-free (`farm/dog-wide`); each script appends
  ...
  suffix is added at ship time (`night-twins.md`).
...
- **+** `light`/`night` name the twins by the theme they serve, matching
