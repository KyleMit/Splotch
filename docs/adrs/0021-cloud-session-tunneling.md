...
summarised in [docs/CLOUD.md](../CLOUD.md); this section is the underlying detail. Note that
`--host` is **not** required in the cloud — the chisel client forwards via `localhost`, so plain
`vite dev` works; the only thing the tunnel needs from Vite is the `allowedHosts` entry below.
