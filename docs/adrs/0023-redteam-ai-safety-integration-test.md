...
* `/api/generate-image` returns **`422`** for a safety refusal (vs `502` for an upstream/empty
  ...
  `aiGenerationState.error: { kind: 'safety', … }` (`aiGeneration.svelte.ts`);
  `AiImageResult.svelte` shows a child-friendly "let's try drawing something else!". As amended by
  ADR-0109, the retired timer harness was replaced by Playwright endpoint mocks invoked through the
  production-flow dev seam; the timeout presentation no longer has E2E coverage.
