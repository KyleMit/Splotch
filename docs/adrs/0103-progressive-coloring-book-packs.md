...
installed ones show: a Save-Data, cellular, or 2G connection still reads the manifest once per
session and publishes the books already in Cache Storage, and disallowing downloads mid-session
cancels an active transfer without hiding what the scan found (issue #1932). Removal cancels/pauses
installation for the current app session, clears both resolution namespaces for the current app
version, and immediately returns the picker to Farm.
