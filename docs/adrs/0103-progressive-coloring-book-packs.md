...
two variants: `full` reads the canonical file; `compact` maps to 240 px thumbnails and 1,152 px
overlays/fills generated by `gen:coloring-responsive`. The manifest also names the starter book and
total bytes per book and variant.
...
`coloringPacks/resolution.ts` selects `compact` or `full` from full-screen CSS dimensions, capped
DPR, and contained 3:2 paper geometry. The compact tier permits at most 1.125× presentation upscale;
phones and low-DPR screens save bandwidth while high-DPR tablets retain canonical pixels. The
manifest separates the canonical logical `path` from `downloadPath`, so every store fetches the
selected bytes but publishes them under the same URL/filename existing WebView consumers use.
...
* **+** Catalog growth increases hosted storage and post-install transfer, not app-store or PWA
  install size.
* **+** Compact devices transfer about 18.2 MiB for all seven background books instead of 28.3 MiB,
  a 35.5% reduction. The same logical inventory and integrity checks cover both variants.
...
* **-** The compact tier can upscale by at most 12.5% on the largest qualifying phones. This is a
  deliberate bandwidth/line-art tradeoff; tablets do not take it.
