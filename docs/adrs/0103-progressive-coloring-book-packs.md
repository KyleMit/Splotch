...
two variants: `full` reads every canonical file; `compact` keeps canonical 400 px thumbnails and
maps overlays/fills to the 1,152 px derivatives generated by `gen:coloring-responsive`. A
`downloadPath` is serialized only when it differs from the logical path and defaults to that path
during parsing. The manifest also names the starter book and total bytes per book and variant.
