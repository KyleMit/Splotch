...
For physical iPad capture, use the established Appium/XCUITest capture entries after the full device
preflight. The `perf:ios:webkit:gates` and `perf:ios:webkit:frames` entries use a legacy inspector
proxy with a documented modern-iOS discovery limitation; they are not evidence that the iPad itself
cannot be automated. Read the [iPad transport guidance](PROFILING-IPAD.md) before selecting one of
those entries.
...
| `npm run perf:android:browser:actions`               | the shared discrete-action plan in Android Chrome on an ADB-connected emulator/device. It launches an owned profiler tab, uses direct CDP trusted touch, evicts browser caches, waits for stable frames, pins an adaptive-sync panel to 60Hz for the sweep (ADR-0143; verified against `dumpsys display` and restored with rotation — an interrupted run can leak the pin, see the campaigns doc), and restores rotation. Use this instead of Android browser automation through Appium, whose UiAutomator2/Chromedriver path can pause frame presentation (ADR-0092)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | the same action-local raw samples and grouped verdicts as `perf:ios:xcuitest:actions` (`actions.json`)                                     |
| `npm run perf:web:webkit`                            | Playwright **WebKit** (the iOS WKWebView engine), production preview                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | engine marks + FPS (no CDP trace)                                                                                                          |
| `npm run perf:ios:webkit:gates`                      | **Legacy inspector-proxy entry; affected by modern-iOS discovery limits.** a **real USB-connected iPad** — the ADR-0066 gates on real WebKit + Apple GPU + 120 Hz ProMotion. Serves the instrumented build, attaches over the WebKit Inspector Protocol (`brew install ios-webkit-debug-proxy`), and drives `/dev/engine`. Needs Safari open on a tab; see `docs/PROFILING-IPAD.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | the per-scenario gates table (`ipad-gates.json`) — no trace, and no Timeline: recording one stays manual                                   |
| `npm run perf:ios:webkit:frames`                     | **Legacy inspector-proxy entry; affected by modern-iOS discovery limits.** the **real screen** on a real USB-connected iPad — the app at `/`, not `/dev/engine`. Frame pacing, input queue delay, paint latency, `pointermove` delivery, finger-up→halo-gone, and undo-history growth, with a CSS A/B sweep over the blend nudge / `mix-blend-mode` / `PointerHalos` and blank-vs-coloring-page. `--drive` runs it with no human hand; see `docs/PROFILING-IPAD.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | four tables + worst-frame forensics + the raw tables (`real-screen.json`), re-readable with `perf:analyze:frames`                          |
...
| `npm run perf:ios:bundled:frames`                    | the same trusted physical-iPad path against the installed app's bundled `capacitor://localhost` page. The harness writes its complete nonce-bound report through Capacitor Preferences and the Mac pulls the app-container plist with `devicectl`; no probe host or mixed-content HTTP is involved. Build and install with `npm run perf:build:cap` first. `--hand-input --seconds=N` is an experimental human-finger control with WDA still attached, not a clean coalescing witness until an attached-vs-detached control clears that assumption                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | the same raw artifact, plus verified bundled-page identity, runtime UA, report-channel byte count, and full-table counts                   |
| `npm run perf:ios:xcuitest:actions`                  | discrete UI actions through the same Appium path — drawer, palette, brushes, stroke width, responsive Settings, themes, coloring-grid open/scroll/select, screenshot, undo, drag-to-clear, and rotation. It supports mobile web or a native Capacitor WebView, retains one warmup plus three scored repeats by default, records both frame clocks inside the page (ADR-0163 — the scheduled rAF stamp is scored, the actual callback time is attribution, reported per action as a non-gating `frameStamps` figure), and fails uncaptured input or the 20 ms P95 / 33.5 ms first/worst-frame gates — a worst-frame breach counting only when two of the three scored repeats show it (ADR-0156) — (rotation first frames on iPad Safari are declared N/A rather than gated — ADR-0142: `resize` lands in the same rendering turn as the first frame there). Where the target keeps web pack storage, a sweep that includes the `coloring` group first settles the installed coloring books, before its first measured action: it opens the picker to release a first visit's engagement-gated downloads, waits for every book in the served catalog (bounded, failing with the missing books named), confirms the picker lists them all, and records the count as `actionPlan.context.listedColoringBooks`, which the stable-plan check then holds equal across repeats. `--actions=` focuses one family; `--report-only` ranks a broken sweep without stopping | action-local raw samples + grouped first-frame/readiness/frame P95/max verdicts (`actions.json`)                                           |
| `npm run perf:web:actions`                           | the same full action plan and scorer in Playwright on a local desktop. Defaults to WebKit at 1512×982@2×; use `--engine=chromium\|firefox` for cross-browser comparisons and `--headed` when the real Mac presentation path matters. `--url=` can target an externally served historical build, keeping the current runner and input plan identical across an architecture comparison. Each capture runs in a throwaway browser profile rather than an ephemeral context, so Cache Storage carries across the per-sweep reloads as it does in a real browser (an ephemeral WebKit context empties it on reload unless a service worker is alive)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | the same action-local raw samples and grouped verdicts (`actions.json`)                                                                    |
...
| `npm run perf:web:undo:webkit`                       | the same 7 scenarios in Playwright **WebKit** — the engine family the iOS app ships. **Scores the commit gate** against `COMMIT_GATE_MS` — always reported, non-zero exit only while `COMMIT_GATE_ENFORCED` is on; no throttle                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | engine marks (no CDP trace, no JS-heap table) **+** the same per-scenario tables                                                           |
| `npm run perf:web:undo:webkit:fast`                  | the post-merge subset (ADR-0100): `multi-finger` (the sole multi-pointer exerciser) + `crayon-scribbles` (the sole mid-stroke pass-split exerciser), run on pushes to `main`. The named script owns the set; CI never repeats its scenario list. Both scenarios score raw commit P95 against the unchanged budget; a breach must recur in a confirmation pass (ADR-0140). Advisory while `COMMIT_GATE_ENFORCED` is off — measured and reported, does not fail the job                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | raw gate evidence, snapshot capture/crop attribution, and the same per-scenario tables                                                     |
...
| Gate    | none — its ms are advisory                                                | raw P95 for every scenario, vs `COMMIT_GATE_MS`                             |
...
Both tiers score raw commit P95. The fast set preserves the sole multi-pointer path in
`multi-finger` and the sole mid-stroke pass-split path in `crayon-scribbles`. The crayon host
control is reported but never divides the score (ADR-0140). A first-pass breach is remeasured; a
missing or unevaluable confirmation never acquits it. ADR-0158 adds the separate fresh-runner
comparison.
...
`undo.measures` distributions, including `engine.undoPatchCapture` / `engine.undoPatchCrop` /
`engine.undoInkMotion`, in both `scenarios` and `confirmations`. Crop is included in commit during
drawing and can run during undo/repaint: do not add nested totals. Each phase's `harnessWallMs` also
includes Playwright round trips and driver-side payload serialization/transfer; it is neither
page-only latency nor a presentation measurement. Capture is timed around its `drawImage` and is not
confined to `engine.draw`: progressive clear-capture callbacks and repaint can copy outside a queue
drain. Crop includes allocation and the copy. Neither proves CPU execution versus a GPU-process
wait. Moving that wait into drawing can make commit green without improving latency. The
...
`docs/scratchpad/perf/2026-07-22-draw-profile/`.
...
* **iOS** `perf:web:webkit` profiles the WebKit *engine*, not the Simulator app. For device-accurate
  ...
  are coarse.
* **Real iPad** (the highest-fidelity target — real WebKit + GPU + 120 Hz ProMotion): use
  **`npm run perf:ios:xcuitest:screen`** (ADR-0084) for automated trusted-touch drawing and
  **`npm run perf:ios:xcuitest:actions`** for discrete actions. The engine-gates entry
  **`npm run perf:ios:webkit:gates`** (ADR-0079) uses the legacy inspector proxy and has the
  modern-iOS discovery limitation described above. Its workload differs from real-screen capture;
  the [iPad runbook](PROFILING-IPAD.md) explains the manual engine-probe path and the outstanding
  transport migration. **A Timeline recording remains a manual Safari Web Inspector flow** — the
  protocol's `Timeline` domain is not the export shape `perf:analyze:web-inspector` parses.
