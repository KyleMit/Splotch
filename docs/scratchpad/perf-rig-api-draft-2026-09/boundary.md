...
| `input-fidelity.mjs`         | splits | The verdict engine, check vocabulary and tri-state move; `RUNTIME_EXPECTATIONS` and the two thresholds stay as `splotch/gates.ts`, and a thin binding at the old path keeps the one- and two-argument signatures the seventeen corpus-test call sites use.       |
...
| `campaign-ledger.mjs`        | moves  | The status vocabulary becomes a closed union with an exit-code column; `attemptsFor`, `nextAction` and the retry policy move as the package's ledger semantics.                                                                                                  |
...
| `campaign-plan.mjs`          | splits | Three files in one. The host-reachability classifier (L378–496), the plan expander, `artifactMatchesRuntime` (as `runtimeOf`) and the gesture-plan and prime readers move as typed evidence; the target registry, item vocabulary and artifact-path scheme stay. |
...
| `undo-driver.mjs`            | stays  | Its in-page function source becomes `controls.undo.drive`, a `PageFunction` the bootstrap and the WebDriver path both call.                                                                                                                                      |
...
| `lib/page-bootstrap.mjs`      | splits | The 17-step sequence becomes the package's bootstrap compiler over the procedure vocabulary, the prime protocol and the plan-poll handshake; every selector, hook and procedure it embeds is read from the `AppContract`. |
...
| `web/capture-web-mount.mjs`            | stays  | Thin: a `session` scenario with no beats and a load window.                                                                                                                                                      |
...
| `web/replay-input-recording.mjs`       | stays  | A Splotch script over `openChannel` with a trace; the replayer is a product API adapter.                                                                                                                         |
| `web/run-undo-scenarios.mjs`           | stays  | A Splotch script over `openChannel`, `evaluateCommit` and the fast-set helpers (`splotch/engine-scripts.ts`).                                                                                                    |
...
| `ios/capture-xcuitest-screen.mjs`      | splits | The hand-rolled WebDriver client, context selection, native bounds, cache eviction, SW block and gesture-pass driving move as the `appium` transport and `ActionDriver`; geometry, brush ids, storage seed stay. |
...
| `gen-performance-matrix.mjs`           | stays  | The model and the renderer are Splotch's (the renderer produces the scrapbook page ADR-0059 keeps in one place); it calls `inspectCell` and the scorers.                                                         |
| `check-matrix-staleness.mjs`           | stays  | Repo-layout semantics end to end.                                                                                                                                                                                |
...
reads the legacy corpus through Splotch's own upgrader, and calls the package's scorers with the
field names the corpus already carries; the acceptance test for the scorers moving is that those
tests pass with only their import lines changed.
