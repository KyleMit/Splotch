...
| Proving the setup           | Eighteen guards and twelve verdicts, each recorded in the artifact, applicable by transport, page delivery and scenario kind; a refusal envelope; a preflight that reuses foreign listeners | Build command, output directory, identity strategy, build variants per delivery; which guards a diagnostic capture may tolerate |
| Diagnosing                  | Input and rotation verification over the split transport, the probe-overhead study, and the frames, Chrome-trace and Web Inspector analyzers                                                | The gesture plan, the fidelity table and the marks contract they narrate with                                                   |
