  ...
  for (let top = 0; top + win <= info.height; top += 10)
    for (let left = 0; left + win <= info.width; left += 10) {
      ...
      for (let y = top; y < top + win; y += 3)
        for (let x = left; x < left + win; x += 3) { const v = data[y * info.width + x]; s += v; s2 += v * v; n++; }
...
async function ampDiffBuf(a, b, gain) {
  ...
  return sharp(out, { raw: { width: x.info.width, height: x.info.height, channels: 3 } }).png().toBuffer();
}
...
for (const q of [75, 92]) {
  ...
  cells.push(await stack([
    svgLabel(CW, 52, [`q${q}`, { t: `${(enc.length / 1024).toFixed(0)} KB per cover`, size: 14, weight: 400, fg: '#6b7280' }],
      { size: 20, fg: q === 75 ? '#991b1b' : '#166534' }),
    svgLabel(CW, 24, ['what the app actually shows — 141 px'], { size: 13, weight: 400, fg: '#6b7280' }),
    await center(await sharp(enc).resize(DISPLAY, DISPLAY).png().toBuffer(), CW, DISPLAY),
    svgLabel(CW, 30, [`${ZF}x zoom on the busiest texture`], { size: 13, weight: 400, fg: '#6b7280' }),
    await center(await zoomCrop(enc, { ...crop, factor: ZF }), CW, ZW * ZF),
  ], { gap: 4 }));
}
const diff = await ampDiffBuf(encs[92], encs[75], 6);
cells.push(await stack([
  svgLabel(CW, 52, ['q75 vs q92', { t: 'what q75 threw away (x6)', size: 14, weight: 400, fg: '#6b7280' }], { size: 20, fg: '#6b7280' }),
...
await writeFile('.viz/out/e7-covers.png', await stack([
  svgLabel(bw, 54, ['gen-style-covers q75 — the artifacts are real; the 141 px display size is what hides them'], { size: 21, bg: '#f3f4f6' }),
  body,
], { gap: 10, bg: '#e5e7eb' }));
