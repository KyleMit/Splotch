...
async function meanAbsErr(a, b) {
  ...
  let s = 0, worst = 0;
  for (let i = 0; i < x.data.length; i++) { const d = Math.abs(x.data[i] - y.data[i]); s += d; if (d > worst) worst = d; }
  return { mean: s / x.data.length, worst };
}

const TILE = 210;
const tiles = [];
let sum90 = 0, sum92 = 0, b90 = 0, b92 = 0;
for (const f of picks) {
  const src = await sharp(f).png().toBuffer();
  const e90 = await encodeQ(src, 90), e92 = await encodeQ(src, 92);
  const m90 = await meanAbsErr(src, e90), m92 = await meanAbsErr(src, e92);
  sum90 += m90.mean; sum92 += m92.mean; b90 += e90.length; b92 += e92.length;
  const name = f.replace('tools/asset-gen/fill-src/', '').replace(`.${variant}.raw.webp`, '');
  tiles.push(await stack([
    await sharp(src).resize(TILE, TILE, { fit: 'contain', background: '#ffffff' }).png().toBuffer(),
    svgLabel(TILE, 22, [name], { size: 12, weight: 600, fg: '#374151' }),
    svgLabel(TILE, 20, [`q90 err ${m90.mean.toFixed(2)} vs q92 ${m92.mean.toFixed(2)} /255`], { size: 11, weight: 500, fg: '#6b7280' }),
  ]));
  console.log(`${name}: q90 mean ${m90.mean.toFixed(2)} worst ${m90.worst} | q92 mean ${m92.mean.toFixed(2)} worst ${m92.worst}`);
}
const rows = [];
...
await writeFile(`.viz/out/${outName}`, await stack([
  svgLabel(bw, 52, [`${gen} q90 — 12 of the 96 ${variant} raw fills it encodes`,
    { t: 'err = mean absolute pixel error vs the lossless source, out of 255', size: 14, weight: 400, fg: '#6b7280' }], { size: 20, bg: '#f3f4f6' }),
  body,
], { gap: 10, bg: '#e5e7eb' }));
