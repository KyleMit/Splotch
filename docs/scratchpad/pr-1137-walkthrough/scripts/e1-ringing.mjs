...
for (const c of CASES) {
  ...
  cells.push(
    await stack([svgLabel(WIN * ZOOM, 34, [`error vs q50 (x${GAIN})`], { size: 17 }), dz])
  );
  const mSrc = await midtoneFraction(src),
    mHi = await midtoneFraction(hi),
    mLo = await midtoneFraction(lo);
  console.log(
    `${c.tag}: midtone src ${(mSrc * 100).toFixed(2)}%  q92 ${(mHi * 100).toFixed(2)}%  q50 ${(mLo * 100).toFixed(2)}%`
  );
