...
# that quotes a LAN origin loses the address the same way.
{
  ...
  buildEntry, buildDigest, productCommit, refreshRatePin, frameStampEpoch, pageEntries, serviceWorkerRegistration,
  gateAllowances,
  actionPlan: (.actionPlan | walk(if type == "string" then gsub("//[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+"; "//<host>") else . end)),
  coloringPreparation,
  actionGroups: .actions,
  summaries: [.summaries[] | {label, passed, count, totalCount, activation, frames, firstFrame, ready, frameSamples, frameStamps}],
  samples: [.samples[]? | {label, repeat, warmup, activation, eventType, trusted, actionAt, firstFrameMs, readyMs, frameStampEpoch,
            postActionFrameColumns: ["gapMs", "startFromActionMs", "endFromActionMs", "visualEffectsActive", "ranFromActionMs", "actualGapMs"],
            postActionFrameRows: [.postActionFrames[]? | [.gapMs, .startFromActionMs, .endFromActionMs, .visualEffectsActive, .ranFromActionMs, .actualGapMs]],
