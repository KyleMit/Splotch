...
//   node docs/scratchpad/perf/2026-09-20-animation-historical-r2/compare.mjs [--all]
import { existsSync } from 'node:fs';
import { join } from 'node:path';
import { SETS, armOf, rawMaxima, readRun, rescore, root, scoredMaxima } from './lib.mjs';
...
for (const [directory, target, captures] of SETS) {
  const runs = captures.map((capture) => ({ capture, run: existsSync(join(root, directory, `${target}-${capture}.actions.reduced.json.gz`)) ? readRun(directory, target, capture) : null }));
  const labels = runs.find((entry) => entry.run)?.run.actionPlan.applicableLabels ?? [];
  console.log(`\n## ${directory}/${target}`);
  for (const label of labels) {
    const cells = runs.map(({ capture, run }) => {
      if (!run) return { capture, text: 'hole', passed: null };
