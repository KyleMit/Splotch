...
`COLORING_SCROLL_OFF_CENTRE_CSS_PX` starts the native-touch swipe 2 CSS pixels off centre, scaled to
the route's native units (6 physical pixels on this phone). That is inside the narrowest picker
gutter, `--space-2` at 8 CSS pixels wide, so the swipe still begins on the gutter between the two
tile columns where the centred swipe used to land (CSS x 182 against 180). The CDP touch route is
left on the centre: it does not pass through InputDispatcher's occlusion check. A scroll timeout now
also reports what the page received, which is what made this diagnosable in one run.

The first version of the offset was 8 units in whatever the route's native unit was. Review showed
that on a CSS-pixel route it left the gutter and landed on a tile. The `first-offset-*` runs below
used that version on the phone, where 8 physical pixels is 2.67 CSS pixels and still in the gutter.
...
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `retained-log/`                                       | Script and output re-deriving, from the Appium server log, that all nine centred swipes (four that scrolled overnight, four overnight timeouts, this unit's control as S13) share one gesture, WebView rect, dialog geometry, viewport and driver response | machine-checked from the saved output; the raw log names the device and stays local, so the output is a snapshot a reader cannot regenerate without it |
| `control-centre/`                                     | Fresh failure on the centred swipe with the new timeout state (`uncaptured`, dialog scrollable, `scrollTop` 0), the logcat verdict, and the overlay windows                                                                                                | machine-checked, except `overlay-windows.txt`, an operator reduction of a local `dumpsys input` listing                                                |
| `first-offset-coloring/`, `first-offset-full-groups/` | Original command on the first offset: `--actions=coloring`, then the overnight run's eleven groups (24 actions). 4/4 sweeps pass in both                                                                                                                   | machine-checked; no logcat file was saved for these, the zero drop counts were read at the terminal                                                    |
| `final-coloring/`, `final-full-groups/`               | The same two captures on the final harness. 4/4 sweeps pass in both, and the saved logcat filter is empty                                                                                                                                                  | machine-checked                                                                                                                                        |
...
* One control and four treatment captures. The overlay state is the device owner's and was not
...
* The iPad XCUITest route shares the native-touch offset and was not captured here. Android Chrome
  uses CDP touch and is unchanged.
