...
|                       31 (9 / 22) | `design-foundations`: dark dialog header contrast                 | Live spec race. [CI run 36212685522](https://github.com/KyleMit/Splotch/actions/runs/36212685522) sampled contrast 2.12 during the icon-fill theme transition, then passed on retry. Retry the sampled style assertions until the rendered state meets the existing threshold; 30 targeted passes at four workers before the change, 30 at ten workers after. |
...
|                         3 (1 / 2) | `privacy`: contents row scrolls to pin                            | Still open. [CI run 36107112648](https://github.com/KyleMit/Splotch/actions/runs/36107112648) left the last link outside the viewport after the panel bottom reached the fold. The assertion is in the shared `contents-helpers.ts`; one trunk event was on 2026-09-23 and the latest branch event on 2026-09-25.                                             |
...
The four changes in this run are **test races**. No confirmed product or infrastructure defect was
found in their failure logs. The three unexplained signatures stay active: the full-suite sweep and
30 targeted repetitions of each relevant test did not reproduce them, so there is no basis for a
...
* CI retry history is not a list of current defects. The highest-ranked three signatures were
  already fixed by the 2026-09-22 hunt; two later branch-only failures used heads that predated
  their fixes. Compare event SHAs with the fix commit before reopening them.
...
* Thirty isolated repetitions of the original dark-header test passed at four workers despite 31
  masked CI events. Isolation did not reproduce CI contention, so the CI failure assertion and the
  ten-worker post-fix amplifier are the evidence; a clean streak is not proof of zero flakiness.
...
The three unresolved signatures were amplified together at the same ten-worker load, on port 5197,
with retries disabled and 30 repetitions each. The landscape privacy panel, both Parent Center
dismissals, and the scroll-cue specimen passed **120/120 executions**. Their historical CI failures
remain leads for the next digest; this clean local sample does not establish a zero failure rate.
