...
const COLORING = 'web/static/coloring', FILLSRC = 'tools/asset-gen/fill-src';
const outDir = process.argv[2];
await mkdir(outDir, { recursive: true });
const PAGES = ['creatures/dragon-wide', 'dinosaur/trex-wide', 'farm/cow-wide', 'nature/ladybug-wide',
...
for (const page of PAGES) {
  ...
  const analysis = await prepareNightAnalysis(await readFile(join(FILLSRC, `${page}.night.raw.webp`)), source ?? pen);
  await writeFile(join(outDir, page.replace('/', '__') + '.webp'), await punchNightCandidate(analysis, source ?? pen));
}
