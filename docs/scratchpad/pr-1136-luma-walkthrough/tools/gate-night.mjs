...
import {
  ...
  fillRgbAt,
  sourceLumaAt,
} from '../tools/asset-gen/lib/night-analysis.mjs';
...
async function scaleBar(value, bar) {
  ...
  for (let y = 0; y < H; y++)
    for (let x = 0; x < W; x++) {
      ...
      let c = [g, g, g];
      if (y < 14) c = [13, 17, 23];
      const vx = Math.round((value / 255) * (W - 1)),
        bx = Math.round((bar / 255) * (W - 1));
...
for (const page of PAGES) {
  ...
  bgPanels.push(
    {
      png: await fit(await png(t1.data, w, hh)),
      label: `${page} night fill`,
      sub: 'fill-src *.night.raw.webp',
    },
    {
      png: await fit(await png(paint, w, hh)),
      label: 'open background kept',
      sub: `pink = masked out (${(100 - (lumas.length / n) * 100).toFixed(0)}% of frame)`,
    },
    {
      png: await scaleBar(bgLuma, 60),
      label: `median background luma = ${bgLuma.toFixed(1)}`,
      sub: '0-255 scale · white tick = this page · red = the 60 bar',
      color: bgLuma <= 60 ? '#7ee787' : '#ff7b72',
    }
  );
  ...
  const w2 = s2.info.width,
    h2p = s2.info.height,
    n2 = w2 * h2p;
  const outline = new Uint8Array(n2);
  let srcCount = 0;
  for (let i = 0; i < n2; i++)
    if (s2.data[i] < OUTLINE_INK_CUTOFF) {
      outline[i] = 1;
      srcCount++;
    }
  ...
  for (let i = 0; i < n2; i++) {
    ...
    else if (white[i] && !blobs[i])
      c = [88, 166, 255]; // thin white, near a real line — allowed
    else if (white[i])
      c = [60, 70, 90]; // thick white blob — a pale fill, ignored
    else c = [16, 20, 28];
    drift[i * 3] = c[0];
    drift[i * 3 + 1] = c[1];
    drift[i * 3 + 2] = c[2];
  ...
  driftPanels.push(
    {
      png: await fit(await png(t2.data, w2, h2p)),
      label: `${page} night fill`,
      sub: 'the gate’s input',
    },
    {
      png: await fit(await png(whitePng, w2, h2p)),
      label: 'luma > 185 & chroma < 45',
      sub: 'every "white outline-ish" pixel',
    },
    {
      png: await fit(await png(drift, w2, h2p)),
      label: `classified — drift ratio ${(added / srcCount).toFixed(4)}`,
      sub: `blue = near a source line (ok) · grey = thick pale fill (ignored) · red = invented (${added}px)`,
      color: added / srcCount < 0.004 ? '#7ee787' : '#ff7b72',
    }
  );
}
...
await grid({
  ...
  subtitle:
    'The open background is flood-filled from the border through the line art, then the MEDIAN of luma(r,g,b) over exactly those pixels decides whether the page reads as night. That per-pixel call is one of the five this PR moved onto the shared helper.',
  panels: bgPanels,
  cols: 3,
  cell: 300,
  out: `${OUT}/E1-night-bg.png`,
});
await grid({
  title: 'Gate D2 — night-scores.mjs scoreDrift()',
  subtitle:
    'luma(r,g,b) > 185 with low chroma marks a pixel "white outline-ish". Thin white far from any source line is an outline the model invented. Same helper, different bar.',
  panels: driftPanels,
  cols: 3,
  cell: 340,
  out: `${OUT}/E2-night-drift.png`,
});
console.log('done');
