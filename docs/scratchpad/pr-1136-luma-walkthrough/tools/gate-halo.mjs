...
import {
  ...
  HALO_DARK,
  HALO_PROTECT_BLACK,
} from '../tools/asset-gen/lib/night-halo.mjs';
...
const PAGES = process.argv.slice(2).length
  ...
  : ['vehicles/train-wide', 'objects/teddy-wide', 'creatures/dragon-wide'];
...
for (const page of PAGES) {
  ...
  for (let p = 0, i = 0; p < n; p++, i += 3)
    if (luma(line[i], line[i + 1], line[i + 2]) < OUTLINE_LUMA_THRESHOLD) mask[p] = 1;
  const refRgb = Buffer.from(rawRgb);
  bleedUnderMask(refRgb, dilateMask(mask, w, h, REF_DILATE), w, h);
  const { data: ship } = await sharp(shipped)
    .removeAlpha()
    .raw()
  ...
  for (let p = 0; p < n; p++) {
    ...
    delta[p * 3] = v;
    delta[p * 3 + 1] = Math.round(v * 0.45);
    delta[p * 3 + 2] = Math.round(v * 0.2);
    let c = [16, 20, 28];
  ...
  panels.push(
    {
      png: await fit(await png(ship, w, h)),
      label: `${page} shipped night`,
      sub: 'what the app actually paints',
    },
    {
      png: await fit(await png(refRgb, w, h)),
      label: 'reference punch',
      sub: 'mask dilated 4px, then bled — "clean" fill',
    },
    {
      png: await fit(await png(delta, w, h)),
      label: 'rimΔ = luma(ref) − luma(shipped)',
      sub: 'two luma() calls subtracted, ×3 gain',
      color: '#ffd166',
    },
    {
      png: await fit(await png(cls, w, h)),
      label: `haloScore = ${score.haloScore}`,
      sub: `red = halo px in rings 1-2 (${halo}/${bandN}) · bar: <= 2`,
      color: score.haloScore <= 2 ? '#7ee787' : '#ff7b72',
    }
  );
}
await grid({
