...
for (const page of PAGES) {
  ...
  for (let p = 0; p < n; p++) {
    if (ink[p]) inkN++;
    const c = ink[p] ? [255, 209, 102] : [16, 20, 28];
    inkPng[p * 3] = c[0];
    inkPng[p * 3 + 1] = c[1];
    inkPng[p * 3 + 2] = c[2];
  }
  ...
  let fN = 0;
  for (let p = 0; p < n; p++) {
    const on = lumas[p] < OUTLINE_INK_CUTOFF;
    if (on) fN++;
    const c = on ? [88, 166, 255] : [16, 20, 28];
    fenced[p * 3] = c[0];
    fenced[p * 3 + 1] = c[1];
    fenced[p * 3 + 2] = c[2];
  }
  panels.push(
    {
      png: await fit(await sharp(buf).removeAlpha().png().toBuffer()),
      label: `${page}.outline.webp`,
      sub: 'one file feeds BOTH paths below',
    },
    {
      png: await fit(await png(inkPng, w, h)),
      label: `ink[] — luma() < ${OUTLINE_LUMA_THRESHOLD}`,
      sub: `MOVED to the shared helper · ${((inkN / n) * 100).toFixed(1)}% of pixels`,
      color: '#ffd166',
    },
    {
      png: await fit(await png(lumaPng, w, h, 1)),
      label: 'analysis.luma — libvips channel',
      sub: 'FENCED, untouched by this PR',
    },
    {
      png: await fit(await png(fenced, w, h)),
      label: `that channel < ${OUTLINE_INK_CUTOFF}`,
      sub: `what chalk-ink-diff thresholds · ${((fN / n) * 100).toFixed(1)}% of pixels`,
      color: '#58a6ff',
    }
  );
}
