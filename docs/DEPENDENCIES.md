...
| Playwright browsers (Chromium, WebKit) | `test.yml` (`npx playwright install` + `npm run test:e2e:deps`) | browser **binaries** downloaded by the `@playwright/test` package (in `package.json`) and cached by lockfile version in CI; their apt **system libraries** can't be cached, so `scripts/install-browser-deps.mjs` installs a trimmed set on every run | keep — versioned by the npm package; the binaries are a separate download, not a separate dep                                                         |
