...
sweep landed as artifacts the campaign runner itself accepts (the same inspection
`perf:campaign:status` reports, so a `blocked-coverage` sweep or an unscoreable drawing reads as
missing):
