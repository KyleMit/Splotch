...
| `drawing/crayonBrush.ts`        | The waxy crayon free-draw default (ADR-0065): a deterministic paper-tooth field + per-colour tinted `repeat` tooth pattern that `renderOp` strokes for `brush: 'crayon'` ops, so overdrawn same-colour passes build up at constant hue. Owns the named variants + the `setCrayonParams` dev-tuning seam. Tooth math unit-tested.                                                                                                          |
