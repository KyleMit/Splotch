...
| `hexPickerLayout.ts` | Pure layout logic for the color picker's honeycomb: the 9×9 palette (9 hue families × 9 shades) plus the responsive rules — the viewport's constrained axis trims shades (never hues), landscape transposes the grid so families become columns, and survivors are picked by even spread across the ramp. `ColorPicker.svelte` renders whatever this returns; the hex geometry CSS stays in the component. |
