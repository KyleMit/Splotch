...
| `drawing/crayonBrush.ts`        | The crayon brush's paint source (ADR-0065/0068): a tileable paper-tooth field driving per-(colour,pass) repeating `CanvasPattern`s (phase-shifted by a stored per-stroke seed) and, behind `CrayonOptions.dabs`, the soft-alpha dab sprites + paper-anchored punch tile of the dab-stamp deposit. Owns the tunable knobs + the dev A/B seam.                                                                               |
