...
* **Tooling findings** — from `/session-audit` (categories `[Traversal]` / `[Execution]` / `[Docs]`
  ...
  cloud-session workflow** rather than to production code: a skill under `.claude/skills/`, a
  path-scoped rule in `.claude/rules/`, a `CLAUDE.md` note, a `docs/*` reference, an ADR,
