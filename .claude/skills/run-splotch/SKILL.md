...
color palette down the left, the Parent Controls button top-right, and (with `--draw`) a purple
zig-zag stroke on the canvas. A blank canvas with no stroke means the draw flow regressed.
