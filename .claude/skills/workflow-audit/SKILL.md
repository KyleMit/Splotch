...
2. **Project config** — `.claude/settings.json`, `.claude/settings.local.json`, `.claude/rules/*`,
   ...
   `tools/ruler/lib/direct-provider-skills.mjs`; this Claude-focused audit may inspect and edit only
   a declared Claude package, never synthesize or sync another provider's package.
...
4. **Benchmark** — do a web search for current (this year) Claude Code best practices:
   CLAUDE.md/rules, skills, hooks, subagents, MCP, model tiering, `/clear` vs `/compact`. Compare my
   setup against the modern stack and identify the real gaps (not generic advice).
