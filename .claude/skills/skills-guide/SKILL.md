...
(`.github/workflows/dependabot-review.yml`, `docs/DEPENDABOT.md`, ADR-0077), which posts an advisory
verdict but never merges. It pairs with `dependency-update-audit` in the audit table above and the
two do not overlap: that skill picks packages the repo is behind on and drives the bumps itself,
this one triages PRs Dependabot has already opened.
