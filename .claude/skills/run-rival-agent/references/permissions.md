...
the launcher and the broker CLI run straight from the checkout — the reason the Codex-side package
needs an installer is Codex's sandbox, which Claude Code does not impose here. A Claude Code on the
