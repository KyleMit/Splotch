...
* **Flake-resistance (worker count is derived from the machine — capacity is `cores / 2`; local sits
  ...
  swallows the gesture (`settleFlyIn` in `tests/helpers.ts`, issue \#665); drive strokes through
  `draw`/`dragStroke`, which pace their samples inside the engine's dropped-pointer threshold — a
  hand-rolled run of far-apart `mouse.move`s gets read as a lifted finger and paints a stub of the
  stroke; and verify a fix with `--repeat-each=10`, never in isolation. Full checklist with
