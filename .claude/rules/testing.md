...
* **Flake-resistance (worker count is derived from the machine — `cores / 2` locally, `cores` on CI,
  ADR-0078 — so specs share the CPU):** never assert on a single interaction against a lazily-wired
  control — wrap open-then-assert in `expect(...).toPass()` or reuse a retrying helper
