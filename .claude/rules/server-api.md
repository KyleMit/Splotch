...
* Credential-oracle endpoints (`verify-access-code`, `verify-key`, and comparable negative
  credential checks) deliberately return HTTP `200` with `{ ok: false, error }` for ordinary failed
  verification so validity is not disclosed through the status. Non-oracle request validation
  retains `4xx` responses with the same body shape; throttling retains the standard `429`.
