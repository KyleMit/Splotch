...
Dependabot rebases the next wave needed went unanswered while `main` was broken), and repair by
deleting the duplicate blocks rather than running `pnpm install`, which on that tree also
re-resolved openai, `@google/genai`, and prettier to releases nobody had reviewed. The simulation
also moved from `checkout -B` plus `branch -D` to a `merge-tree`/`commit-tree` chain, because the
branch delete is denied in permission-gated sessions and a collision left the old form mid-merge.
...
* **Overlap with the automated review** (`dependabot-review.yml`, ADR-0081, merged as \#598 during
  ...
  **2026-09-25:** the workflow did post on all twelve PRs of that batch, every one an APPROVE. It
  also approved the lighthouse bump whose CI then failed on the pinned page-load baseline, because
  it reviewed before CI finished. So its verdicts stay an input, and CI is still read separately.
