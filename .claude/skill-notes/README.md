<!-- Source: .ruler/skill-notes/README.md.template -->
...
published into every session's context window — the precise outcome this whole tree is built to
prevent, and one that grows with every note added. The mirror script fails rather than mirror a
plain `.md`. Skill forks carry the suffix on their Markdown for the same reason.
...
`.ruler/` source. `burn-down-audits` has independent notes for both providers;
`implement-issue-stack` has a Codex note only; `analyze-session-transcripts` has a Claude note only.
