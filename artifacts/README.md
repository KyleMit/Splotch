...
That design system lives in [`scripts/lib/artifact-chrome.mjs`](../scripts/lib/artifact-chrome.mjs)
(masthead, footer, tokens, cards); the Node generators import it, and the coloring-book proof sheet
mirrors its tokens by eye since `tools/asset-gen/` can't import across that boundary. The landing
[`index.html`](./index.html) is a curated card grid — one card per collection, generated by
[`artifacts:index`](../scripts/lib/artifacts-index.mjs) from a small registry with a graceful
...
`artifact-chrome.mjs` when the chrome changes.)
...
`npm run gen:icons-sheet -- --out artifacts/icons/index.html` after adding or changing an app icon,
then `npm run artifacts:index`.
