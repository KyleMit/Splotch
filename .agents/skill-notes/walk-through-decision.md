...
* **Grounding may be over-specified for small decisions.** "Read the actual code, check the ADRs,
  check `docs/audit-deferred/decisions/`, measure what is cheap to measure" is right for a real
  architectural fork and heavy for "which of these two names". No escape hatch is written in yet, on
  the theory that over-grounding is the cheaper failure. Watch whether it makes small walkthroughs
  slow.
