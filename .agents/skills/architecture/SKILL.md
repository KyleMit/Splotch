...
| `releases.json`                 | Auto-generated from `releases/*.md` by `npm run gen:releases`; consumed by the Updates section in Parent Center (dated release cards, no version numbers).                                                                                                                                                                                                                                                                                |
...
* **Gradient Swatch** - Last color button with rainbow gradient and + symbol; opens custom color
  picker
...
* **Parent Help Button** - Floating button that opens the Parent Center
  * **Parent Center** - Modal for app settings, install guides, and about info. Its body is one flat
    ...
    Buttons, AI Art, Setup Guide, Updates, Submit Feedback, About. Both shells render from the same
    `SECTIONS` list in `parent/sections.ts`, chosen by viewport width (`ParentCenter.svelte`): below
    ~700px a **Hub** list drills into a full-page section with a back arrow; at/above ~700px a
