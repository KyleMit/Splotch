...
| `drawing/crayon.ts`             | The crayon brush's paint (ADR-0065): a deterministic paper-tooth `CanvasPattern` of the stroke colour (opaque peaks, transparent valleys), phase-shifted per stroke so an overlapping same-colour pass fills the tooth and builds up at constant hue. `renderOp` strokes it for a `crayon` op; owns the tooth generation, the per-colour pattern cache, and the `CrayonParams` dev A/B seam.                                              |
| `drawing/magicBrush.ts`         | The magic brush's color sheet + pattern (ADR-0043) — the reveal source (a page's fill or a rainbow), rasterized and sampled as a `CanvasPattern` by `renderOp` for a `magic` op.                                                                                                                                                                                                                                                          |
...
* **Actions Panel** - Bottom-corner panel hosting auxiliary controls
  ...
  * **Crayon Button** (`#crayonButton`) - Toggles the crayon brush (always shown). Lays the active
    color down through a paper-tooth texture that builds up like real wax on a same-color overpass —
    a `crayon`-flagged op whose paint is a tooth-punched `CanvasPattern`, phase-shifted per stroke,
    so undo/eraser/resize/export are free (ADR-0065).
