...
| `drawing/crayonTexture.ts`      | The crayon brush's paper-tooth texture (ADR-0065): a deterministic near-binary tooth tile, tinted per colour, painted as a per-op `CanvasPattern` in `renderOp` for `crayon`-flagged ops. A stored per-stroke `seed` phase-shifts the tooth so overlapping strokes fill each other's grain — same-hue wax buildup — while replaying bit-identically.                                                                                      |
...
* **Actions Panel** - Bottom-corner panel hosting auxiliary controls
  ...
  * **Crayon Button** (`#crayonButton`) - Toggles the crayon brush; always shown. Draws a waxy
    paper-tooth texture that builds up (denser, same hue) where same-colour strokes overlap — a
    `crayon`-flagged op whose paint is a per-op tooth `CanvasPattern` (`crayonTexture.ts`,
    ADR-0065). A colour tap keeps the crayon selected (`exitToDrawing`), so recolouring doesn't drop
    to the pen.
