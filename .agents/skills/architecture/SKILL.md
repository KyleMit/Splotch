...
| `drawing/crayon.ts`             | The crayon brush (ADR-0065): a deterministic paper-"tooth" pattern tinted per colour and stroked `source-over` for a `crayon`-flagged op, with a per-stroke tooth phase that gives same-hue wax buildup. `renderOp()` delegates crayon ops here; params are dev-tunable via the `setCrayonParams` seam.                                                                                                                                   |
