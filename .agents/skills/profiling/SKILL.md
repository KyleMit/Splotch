...
2. **Engine hot paths** — the `Total`/`Avg`/`Max` per operation. Map a hot row to its cause and fix:
   ...
   | `engine.undo` high             | restoring the top snapshot: a blit for a hot raster, decode + blit for a blob entry | entries past the resident byte budget (ADR-0078) decode from their lossless blob; a one-off cost at button-press, not per-frame.                                                                                                                                                                       |
