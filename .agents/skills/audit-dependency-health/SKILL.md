...
version bumps, no installs beyond what's already on disk. Its sibling
`burn-down-outdated-dependencies` is the Dependabot-like workflow that *applies* upgrades; this one
decides whether a dependency *deserves its place at all*. If an entry here concludes a package
should merely be updated, that's a hand-off to `burn-down-outdated-dependencies`, not work for this
run.
...
3. **Note repo entanglements.** Known ones to carry into entries (verify, don't assume):
   ...
   * The Svelte/Vite family and `@capacitor/*` move as coordinated sets (see
     `burn-down-outdated-dependencies`'s landmine list) — replacement cost for any member includes
     the set.
...
the verdict and is explicitly handed to `burn-down-outdated-dependencies`. Do not silently write
...
`burn-down-outdated-dependencies`, same as an npm minor.
